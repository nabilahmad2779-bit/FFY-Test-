...
const InteractiveArchive: React.FC = () => {
  return (
    <section id={NavSection.Laurels} className="relative py-16 md:py-36 bg-[#030707] overflow-hidden border-t border-white/5">
  ...
  return (
    <div className="w-full bg-[#030303] py-16 md:py-24 px-4 md:px-8 relative overflow-hidden min-h-screen flex flex-col items-center justify-center">
      ...
      <div className="max-w-[1400px] mx-auto relative z-10 w-full">
        ...
        <div className="flex overflow-x-auto pb-4 md:pb-0 md:flex-wrap gap-2 md:gap-4 mb-8 md:mb-12 no-scrollbar bg-black/20 md:bg-black/40 p-1.5 md:p-2 rounded-[1.2rem] md:rounded-[2rem] border border-white/5 backdrop-blur-3xl shadow-3xl snap-x">
          {FORTE_EVENTS.map((event) => (
            <button
              key={event.id}
...
const HomeView: React.FC<{ navigate: (path: string) => void; handleDownload: () => void }> = ({ navigate, handleDownload }) => {
  return (
    <>
             ...
             {HALL_OF_FAME.map((member, i) => (
               <ScrollReveal key={i} className={`delay-${i * 100}`}>
                 <div className="group relative flex flex-col items-center text-center h-full">
                   <div className="w-32 h-32 md:w-44 md:h-44 rounded-full overflow-hidden border border-white/5 p-1 mb-4 md:mb-8 group-hover:border-cyan-500/50 transition-all duration-700 shadow-lg active:scale-95 shrink-0 mx-auto">
      ...
      <section className="bg-black py-16 md:py-32 px-5 relative overflow-hidden border-t border-cyan-500/10">
      ...
      <section id={NavSection.Contact} className="bg-black py-16 md:py-32 px-5 relative overflow-hidden border-t border-white/5">
        <div className="max-w-6xl mx-auto relative z-10 flex flex-col items-center w-full">
          <ScrollReveal>
            <h2 className="text-4xl sm:text-6xl md:text-8xl font-heading font-black uppercase italic mb-12 md:mb-24 leading-[0.8] tracking-tighter text-center">Connect <br /> <span className="text-cyan-500 not-italic">Nexus.</span></h2>
          </ScrollReveal>
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen text-white bg-[#030303] text-center selection:bg-cyan-500 selection:text-black w-full flex flex-col transition-all duration-300">
      ...
      <footer className="bg-black py-12 md:py-16 px-5 relative flex flex-col items-center border-t border-white/5">
        <div className="max-w-6xl mx-auto w-full flex flex-col items-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 items-center text-[7px] md:text-[9px] font-black text-zinc-800 uppercase tracking-[0.3em] mt-10 text-center">
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/98 backdrop-blur-3xl text-white flex flex-col p-6 sm:p-12 animate-fade-in text-center overflow-y-auto">
          ...
          <div className="flex flex-col gap-8 items-center flex-1 justify-center py-4">
             <button onClick={() => navigate('/')} className={`text-3xl sm:text-4xl font-heading font-black uppercase italic tracking-tighter transition-all ${currentView === 'home' ? 'text-cyan-500' : 'text-white'}`}>Nexus</button>
             <button onClick={() => navigate('/departments')} className={`text-3xl sm:text-4xl font-heading font-black uppercase italic tracking-tighter transition-all ${currentView === 'departments' ? 'text-cyan-500' : 'text-white'}`}>Departments</button>
          </div>
