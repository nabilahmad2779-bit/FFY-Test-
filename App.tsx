...
const DepartmentDetailView: React.FC<{ dept: any; navigate: (path: string) => void }> = ({ dept, navigate }) => {
  return (
...
const App: React.FC = () => {
  ...
  const renderView = () => {
    // Departments Hub Routing
    if (currentPath === '/departments' || currentPath === '/departments/') {
      return <DepartmentsView navigate={navigate} />;
    }
    
    // Department Detail Routing
    if (currentPath.startsWith('/departments/')) {
      ...
      if (dept) {
        return <DepartmentDetailView dept={dept} navigate={navigate} />;
      }
    }
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 h-16 md:h-20 flex items-center px-4 md:px-0 ${scrolled ? 'bg-black/95 backdrop-blur-xl border-b border-cyan-500/10' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 lg:px-10 w-full flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-10">
            <button 
              onClick={() => navigate('/')} 
              className={`text-[9px] font-black uppercase tracking-[0.3em] transition-colors ${currentPath === '/' ? 'text-cyan-500' : 'text-zinc-500 hover:text-white'}`}
            >
              Nexus
            </button>
            <button 
              onClick={() => navigate('/departments')} 
              className={`text-[9px] font-black uppercase tracking-[0.3em] transition-colors ${currentPath.includes('/departments') ? 'text-cyan-500' : 'text-zinc-500 hover:text-white'}`}
            >
              Departments
            </button>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/98 backdrop-blur-3xl text-white flex flex-col p-6 sm:p-12 animate-fade-in text-center overflow-y-auto">
          ...
          <div className="flex flex-col gap-8 items-center flex-1 justify-center py-4">
             <button onClick={() => navigate('/')} className={`text-4xl font-heading font-black uppercase italic tracking-tighter transition-all ${currentPath === '/' ? 'text-cyan-500' : 'text-white'}`}>Nexus</button>
             <button onClick={() => navigate('/departments')} className={`text-4xl font-heading font-black uppercase italic tracking-tighter transition-all ${currentPath.includes('/departments') ? 'text-cyan-500' : 'text-white'}`}>Departments</button>
          </div>
