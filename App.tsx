...
import { FORTE_EVENTS, HALL_OF_FAME, HERO_IMAGE_URL, COLLABORATIONS, AWARDS, PARTNER_LOGOS, SPONSORS } from './constants';
...
import { 
  ...
  Linkedin, Facebook, Instagram, Download, Settings, Sparkles, ArrowLeft
} from 'lucide-react';
            ...
            <div className="mb-6 md:mb-10">
              ...
              <div className="grid grid-cols-2 gap-x-4 md:gap-x-12 gap-y-3 md:gap-y-6">
                {[
                  ...
                  { label: 'Khulna', key: 'geoRajshahi' as keyof Demographics }, // Reusing keys as in the current data structure
                  { label: 'Barishal', key: 'geoRajshahi' as keyof Demographics },
                  { label: 'Rangpur', key: 'geoRajshahi' as keyof Demographics },
                  { label: 'Mymensingh', key: 'geoRajshahi' as keyof Demographics },
                ].map((div, i) => (
                  <div 
                    key={div.label} 
...
const DepartmentsSection: React.FC<{ onSelect?: (id: string) => void }> = ({ onSelect }) => {
  return (
...
const HomeView: React.FC<{ onNav: (section: NavSection) => void; handleDownload: () => void; setView: (v: string) => void; setDept: (d: any) => void }> = ({ onNav, handleDownload, setView, setDept }) => {
  return (
    <>
      ...
      <section id={NavSection.Analysis} className="py-0 relative w-full overflow-hidden">
        <NeuralNexusCommand onNav={onNav} onDownload={handleDownload} />
      </section>

      <DepartmentsSection onSelect={(id) => {
        const d = DEPARTMENT_LIST.find(x => x.id === id);
        if (d) { setDept(d); setView('dept-detail'); window.scrollTo(0,0); }
      }} />
...
const DepartmentsView: React.FC<{ onSelect: (id: string) => void }> = ({ onSelect }) => {
  return (
    <div className="pt-32 pb-20 animate-fade-in min-h-screen">
      <div className="max-w-7xl mx-auto px-5 text-center">
        <ScrollReveal className="mb-20">
          ...
          <h1 className="text-5xl md:text-8xl font-heading font-black uppercase italic tracking-tighter leading-none mb-8">
...
// --- NEW PAGE: DEPARTMENT DETAIL VIEW ---

const DepartmentDetailView: React.FC<{ dept: any; onBack: () => void }> = ({ dept, onBack }) => {
  return (
          ...
          <div className="lg:col-span-5">
            <ScrollReveal>
              <div className={`w-28 h-28 md:w-40 md:h-40 rounded-[2.5rem] md:rounded-[3.5rem] bg-white/5 flex items-center justify-center mb-10 border border-white/10 shadow-2xl ${dept.colorClass}`} style={{ color: dept.accent }}>
                {React.cloneElement(dept.icon as React.ReactElement, { size: 64, className: "md:w-16 md:h-16" })}
              </div>
            ...
            <ScrollReveal delay={100}>
               <div className="bg-white/[0.02] border border-white/5 p-10 md:p-16 rounded-[2.5rem] md:rounded-[4rem] relative group hover:bg-white/[0.03] transition-colors">
                  <div className="flex items-center gap-6 mb-8">
                     ...
                     <h3 className="text-2xl md:text-4xl font-heading font-black uppercase italic italic tracking-tight text-white">Core Operations</h3>
                  </div>
               ...
               <div className="bg-white/[0.02] border border-white/5 p-10 md:p-16 rounded-[2.5rem] md:rounded-[4rem] relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-6 mb-8">
                       ...
                       <h3 className="text-2xl md:text-4xl font-heading font-black uppercase italic italic tracking-tight text-white">Institutional Impact</h3>
                    </div>
...
const App: React.FC = () => {
  const [activeView, setActiveView] = useState<'home' | 'departments' | 'dept-detail'>('home');
  ...
  const handleNavToSection = (section: NavSection) => {
    if (section === NavSection.Departments) {
    ...
    // Smooth scroll logic for home page anchors
    setTimeout(() => {
  ...
  const renderCurrentView = () => {
    switch (activeView) {
      case 'departments':
        return <DepartmentsView onSelect={(id) => {
          const d = DEPARTMENT_LIST.find(x => x.id === id);
          if (d) { setSelectedDept(d); setActiveView('dept-detail'); window.scrollTo(0,0); }
        }} />;
      case 'dept-detail':
        return <DepartmentDetailView dept={selectedDept} onBack={() => setActiveView('departments')} />;
      case 'home':
      default:
        return <HomeView onNav={handleNavToSection} handleDownload={handleDownloadPortfolio} setView={setActiveView} setDept={setSelectedDept} />;
    }
  };
    ...
    <div className="min-h-screen text-white bg-[#030303] text-center overflow-x-hidden selection:bg-cyan-500 selection:text-black w-full flex flex-col transition-all duration-300">
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 h-16 md:h-20 flex items-center px-4 md:px-0 ${scrolled ? 'bg-black/95 backdrop-blur-xl border-b border-cyan-500/10' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 lg:px-10 w-full flex justify-between items-center">
          <div className="flex items-center gap-2 md:gap-3 cursor-pointer group" onClick={() => { setActiveView('home'); window.scrollTo(0,0); }}>
          ...
          <div className="hidden lg:flex items-center gap-10">
            <button 
              ...
              className={`text-[9px] font-black uppercase tracking-[0.3em] transition-colors ${activeView === 'home' ? 'text-cyan-500' : 'text-zinc-500 hover:text-white'}`}
            >
              Nexus
            </button>
            <button 
              onClick={() => { setActiveView('departments'); window.scrollTo(0,0); }}
              className={`text-[9px] font-black uppercase tracking-[0.3em] transition-colors ${activeView === 'departments' || activeView === 'dept-detail' ? 'text-cyan-500' : 'text-zinc-500 hover:text-white'}`}
            >
              Departments
            </button>
      ...
      <footer className="bg-black py-10 md:py-16 px-5 relative flex flex-col items-center border-t border-white/5">
        <div className="max-w-6xl mx-auto w-full flex flex-col items-center">
          <div className="flex flex-col items-center gap-6 md:gap-8 group cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/98 backdrop-blur-3xl text-white flex flex-col p-6 sm:p-12 animate-fade-in text-center overflow-y-auto">
          ...
          <div className="flex flex-col gap-8 items-center flex-1 justify-center py-4">
             <button onClick={() => { setActiveView('home'); setIsMenuOpen(false); window.scrollTo(0,0); }} className="text-4xl font-heading font-black uppercase italic tracking-tighter text-white hover:text-cyan-500 transition-colors">Nexus</button>
             <button onClick={() => { setActiveView('departments'); setIsMenuOpen(false); window.scrollTo(0,0); }} className="text-4xl font-heading font-black uppercase italic tracking-tighter text-white hover:text-cyan-500 transition-colors">Departments</button>
             <button onClick={() => { handleNavToSection(NavSection.History); setIsMenuOpen(false); }} className="text-2xl font-heading font-black uppercase italic tracking-tighter text-zinc-500 hover:text-cyan-500 transition-colors">The First Step</button>
             <button onClick={() => { handleNavToSection(NavSection.Initiatives); setIsMenuOpen(false); }} className="text-2xl font-heading font-black uppercase italic tracking-tighter text-zinc-500 hover:text-cyan-500 transition-colors">The Archive</button>
          </div>
