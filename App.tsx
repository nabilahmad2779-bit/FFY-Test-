        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {departmentData.map((dept, i) => (
            <ScrollReveal key={i} delay={i * 100}>
              <div className={`h-full group border p-7 md:p-10 rounded-2xl md:rounded-[2.5rem] transition-all duration-500 flex flex-col relative overflow-hidden ${dept.colorClass} hover:brightness-110 shadow-lg`}>
...
const App: React.FC = () => {
  ...
  const handleDownloadPortfolio = () => {
    ...
    // Page 2: About (The First Step) - High Polish
    doc.addPage();
    doc.setFillColor(darkColor[0], darkColor[1], darkColor[2]);
    doc.rect(0, 0, 210, 297, 'F');
    ...
    // Page 3: The Chronicle - Detailed
    doc.addPage();
    doc.setFillColor(darkColor[0], darkColor[1], darkColor[2]);
    doc.rect(0, 0, 210, 297, 'F');
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.setFontSize(10);
    doc.text("STRATEGIC PROGRESSION", 20, 30);
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(32);
    doc.text("THE ARCHIVE", 20, 45);
    
    let yPos = 65;
    FORTE_EVENTS.forEach((event) => {
      // Event Header
      ...
      doc.setFontSize(16);
      doc.setFont("helvetica", "bold");
      doc.text(`${event.name}`, 30, yPos + 5);
      
      doc.setTextColor(grayColor[0], grayColor[1], grayColor[2]);
      doc.setFontSize(10);
      doc.text(event.year, 180, yPos + 5, { align: "right" });
      
      yPos += 12;
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      const splitDesc = doc.splitTextToSize(event.description, 150);
    ...
    // Page 4: Apex Circle (Testimonials)
    doc.addPage();
    doc.setFillColor(darkColor[0], darkColor[1], darkColor[2]);
    doc.rect(0, 0, 210, 297, 'F');
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.setFontSize(10);
    doc.text("RESONANT FIGURES", 20, 30);
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(32);
    doc.text("APEX CIRCLE", 20, 45);
    
    yPos = 65;
    HALL_OF_FAME.forEach((member) => {
      doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      ...
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(9);
      doc.text(member.role, 20, yPos + 6);
      
      doc.setTextColor(180, 180, 180);
      doc.setFont("helvetica", "italic");
      const splitImpact = doc.splitTextToSize(`"${member.impact}"`, 170);
      doc.text(splitImpact, 20, yPos + 15);
      
      yPos += splitImpact.length * 5 + 25;
    });

    drawFooter(4);

    // Page 5: Strategic Allies (Partners) - Professional Grid
    doc.addPage();
    doc.setFillColor(darkColor[0], darkColor[1], darkColor[2]);
    doc.rect(0, 0, 210, 297, 'F');
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.setFontSize(10);
    doc.text("ECOSYSTEM NETWORK", 20, 30);
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(32);
    doc.text("STRATEGIC ALLIES", 20, 45);
    
    yPos = 65;
    doc.setFontSize(18);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.text("Corporate Sponsors", 20, yPos);
    yPos += 12;
    doc.setFontSize(12);
    doc.setTextColor(255, 255, 255);
    SPONSORS.forEach(s => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.text("Partner Network", 20, yPos);
    yPos += 12;
    doc.setFontSize(9);
    doc.setTextColor(200, 200, 200);
    doc.setFont("helvetica", "normal");
    
    PARTNER_LOGOS.forEach((p, idx) => {
      const col = idx % 2 === 0 ? 25 : 115;
      doc.text(`• ${p.name}`, col, yPos);
      if (idx % 2 !== 0) yPos += 8;
      if (yPos > 270) {
        drawFooter(5);
        doc.addPage();
    ...
    // Fix: Use PARTNER_LOGOS.length instead of out-of-scope idx
    drawFooter(PARTNER_LOGOS.length > 30 ? 6 : 5);
  ...
  return (
    <div className="min-h-screen text-white bg-[#030303] text-center overflow-x-hidden selection:bg-cyan-500 selection:text-black w-full flex flex-col transition-all duration-300">
      ...
      <section id={NavSection.Analysis} className="py-0 relative w-full overflow-hidden">
        <NeuralNexusCommand onNav={handleNav} onDownload={handleDownloadPortfolio} />
      </section>
          ...
          <div className="w-full max-w-5xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 items-start text-center md:text-left">
              ...
              <ScrollReveal delay={300}>
                ...
                <button onClick={handleDownloadPortfolio} className="w-full px-7 py-3.5 bg-cyan-500 text-black font-black uppercase tracking-widest rounded-lg hover:bg-white transition-all shadow-xl flex items-center justify-center gap-2.5 text-xs md:text-sm active:scale-95">
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/98 backdrop-blur-3xl text-white flex flex-col p-6 sm:p-12 animate-fade-in text-center overflow-y-auto">
          ...
          <div className="flex flex-col gap-6 items-center flex-1 justify-center py-4">
             {/* Matching Mobile Menu list exactly with the provided screenshot */}
             <button onClick={() => handleNav(NavSection.History)} className="text-3xl sm:text-4xl font-heading font-black uppercase italic hover:text-cyan-400 tracking-tighter leading-none transition-all active:scale-95 animate-fade-in">About Us</button>
             <div className="inline-flex items-center gap-2 px-3 py-1 bg-cyan-500/10 rounded-full border border-cyan-500/20 mb-4">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.3em] text-cyan-500">For A Fortunate Future</span>
             </div>
             {[
               { l: 'Chronicle', v: NavSection.Initiatives },
               { l: 'Apex Circle', v: NavSection.HallOfFame },
               { l: 'The Archive', v: NavSection.Laurels },
               { l: 'Nexus Portal', v: NavSection.Contact },
             ].map((item, i) => (
              <button 
                key={item.l} 
                onClick={() => handleNav(item.v)} 
                className="text-3xl sm:text-5xl font-heading font-black uppercase italic hover:text-cyan-400 tracking-tighter leading-none transition-all active:scale-95 animate-fade-in my-2"
