               ...
               <ScrollReveal key={event.id} delay={i * 100}>
                 <div className={`rounded-[3rem] overflow-hidden border transition-all active:scale-[0.98] ${isDark ? 'bg-black border-white/10 shadow-2xl' : 'bg-white border-slate-200 shadow-xl'}`}>
                    ...
                    <div className="p-10">
                       ...
                       <button onClick={() => onNavigate(`/events/${event.id}`)} className={`w-full py-5 font-black uppercase tracking-[0.2em] text-[11px] rounded-2xl flex items-center justify-center gap-3 active:scale-95 transition-all group border shadow-lg relative overflow-hidden ${isDark ? 'bg-white/5 border-white/20 text-white' : 'bg-slate-100 text-slate-900 border-slate-200'}`}>
  ...
  const renderDepartment = () => {
    ...
    if (isDark) {
        switch (currentDeptId) {
            ...
            case 'acad': return <MobAcadDepartment onBack={() => navigate('/departments')} isDark={isDark} />;
            default: return null;
        }
    } else {
        switch (currentDeptId) {
