...
// Import Light Event Detail Pages
import { MosaicStoriesMobile_light } from './pages/EventsForMobileLight/MosaicStoriesLight.tsx';
import { SpiritualQuestMobile_light } from './pages/EventsForMobileLight/SpiritualQuestLight.tsx';
import { CosmicQuestMobile_light } from './pages/EventsForMobileLight/CosmicQuestLight.tsx';
import { BrushFlashMobile_light } from './pages/EventsForMobileLight/BrushFlashLight.tsx';
...
import { PanelPageMobile } from '../components/MainMenuPages/MainMenuPagesMobile/PanelPageMobile';
...
const MobileHome: React.FC<{ isDark: boolean; onNavigate: (view: string) => void }> = ({ isDark, onNavigate }) => {
  ...
  return (
    <div className="animate-fade-in relative z-10">
       ...
       {/* 1. HERO SECTION */}
       <section className="relative h-[100vh] w-full flex flex-col justify-center px-6 overflow-hidden">
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-black text-white' : 'bg-slate-50 text-slate-900'} transition-colors duration-500`}>
        ...
        <main className={`min-h-screen ${currentView === 'panel' ? '' : 'pt-20'}`}>
            ...
            {currentView === 'panel' && <PanelPageMobile isDark={isDark} />}
            {currentView === 'event-detail' && (
                <>
                    {eventId === 'mosaic-stories' && (isDark ? <MosaicStoriesMobile onBack={() => navigate('/events')} /> : <MosaicStoriesMobile_light onBack={() => navigate('/events')} />)}
                    {eventId === 'spiritual-quest' && (isDark ? <SpiritualQuestMobile onBack={() => navigate('/events')} /> : <SpiritualQuestMobile_light onBack={() => navigate('/events')} />)}
                    {eventId === 'cosmic-quest' && (isDark ? <CosmicQuestMobile onBack={() => navigate('/events')} /> : <CosmicQuestMobile_light onBack={() => navigate('/events')} />)}
                    {eventId === 'brush-flash' && (isDark ? <BrushFlashMobile onBack={() => navigate('/events')} /> : <BrushFlashMobile_light onBack={() => navigate('/events')} />)}
                </>
            )}
        </main>
