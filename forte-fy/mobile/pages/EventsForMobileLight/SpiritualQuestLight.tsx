...
export const SpiritualQuestMobile_light: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const isDark = false;
  ...
  return (
    <div ref={containerRef} className={`min-h-screen ${'bg-[#fffaf0] text-slate-900 selection:bg-amber-500 selection:text-white'} font-literary overflow-x-hidden`}>
      
      {/* BACK BUTTON */}
      <div className="fixed top-6 left-4 z-50">
        <button onClick={onBack} className={`group flex items-center gap-2 ${'text-[#b45309] hover:text-[#d97706] bg-white/60 border-[#b45309]/20 hover:border-[#b45309]/50 shadow-sm'} transition-colors px-3 py-2 rounded-full backdrop-blur-md border`}>
      ...
      {/* --- HERO SECTION --- */}
      <section className={`relative min-h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden border-b ${'border-amber-200/50'}`}>
        ...
        <div className="relative z-20 text-center flex flex-col items-center w-full px-4 pt-20 pointer-events-none">
          <div className="mb-6 flex flex-col items-center gap-2 opacity-90">
             <div className={`text-[10px] font-mosaic font-bold uppercase tracking-[0.4em] ${'text-[#b45309]'} drop-shadow-md`}>Forte-FY Presents</div>
             <div className={`w-12 h-[1px] ${'bg-[#b45309]'}`} />
          </div>
          <div className="relative mb-6 w-full">
             <h1 className={`text-[18vw] font-mosaic font-black leading-[0.9] tracking-normal text-transparent bg-clip-text ${'bg-gradient-to-b from-[#d97706] via-[#b45309] to-[#78350f] drop-shadow-sm'}`}>
                The <br/> Spiritual <br/> Quest
             </h1>
          </div>
          <p className={`text-sm font-literary italic ${'text-[#b45309]'} tracking-wider px-2`}>
      ...
      <section className={`py-20 px-6 relative z-10 ${'bg-[#fffaf0]'}`}>
         <div className="reveal-up text-center space-y-8">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-slate-900'} leading-tight`}>
               <span className={`${'text-[#b45309]'} block mb-1`}>A Journey</span> of Knowledge & Reflection.
            </h2>
            
            <div className={`relative group/text-container space-y-6 text-base font-light ${'text-slate-600'} leading-relaxed max-w-sm mx-auto`}>
               <div className={`absolute -left-6 -top-6 ${'text-[#b45309]/10'} animate-pulse`}>
                  <Sparkles size={40} />
               </div>

               <p className={`transition-all duration-500 ${'hover:text-slate-900'} cursor-default`}>
      ...
      <section className={`py-20 px-4 ${'bg-white'} relative overflow-hidden`}>
         <div className={`absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] ${'from-amber-100/50'} via-transparent to-transparent pointer-events-none`} />
         
         <div className="relative z-10">
            <div className="text-center mb-12 reveal-up">
               <span className={`text-[9px] font-mosaic font-bold uppercase tracking-[0.3em] ${'text-[#b45309]'} block mb-3`}>Daily Guidance</span>
               <h2 className={`text-3xl font-mosaic font-bold ${'text-slate-900'} mb-4`}>Ramadan Reflection Calendar</h2>
            ...
            <div className="flex overflow-x-auto pb-8 gap-4 snap-x snap-mandatory no-scrollbar px-2" style={{ scrollBehavior: 'smooth' }}>
               {CALENDAR_DAYS.map((day, i) => (
                  <div key={i} className={`reveal-up snap-center shrink-0 w-[260px] p-6 rounded-2xl ${'bg-[#fffbf0] border-amber-200 hover:border-[#b45309] shadow-sm hover:shadow-lg'} border transition-all duration-500 group relative overflow-hidden flex flex-col h-[320px]`}>
                     
                     <div className={`absolute top-0 right-0 p-3 ${'opacity-10 group-hover:opacity-20'} transition-opacity`}>
                        <Moon size={80} className={'text-[#b45309]'} />
                     </div>
                     
                     <div className="relative z-10 flex flex-col h-full">
                        <div className="flex justify-between items-start mb-4">
                           <div className={`inline-block px-3 py-1 rounded-full border ${'border-amber-300 bg-amber-50 text-[#b45309]'} text-[10px] font-bold uppercase tracking-widest`}>
                              Day {day.day}
                           </div>
                           {/* Category Marker */}
                           <div className={`text-[8px] font-black uppercase tracking-widest ${'text-slate-400 group-hover:text-[#b45309]'} transition-colors`}>
                              {day.day <= 10 ? 'Mercy' : day.day <= 20 ? 'Forgiveness' : 'Salvation'}
                           </div>
                        </div>

                        <h3 className={`text-xl font-mosaic font-bold ${'text-slate-900 group-hover:text-[#d97706]'} mb-4 leading-tight transition-colors`}>
                           {day.title}
                        </h3>
                        
                        <ul className="space-y-3 mt-auto">
                           {day.tasks.map((task, idx) => (
                              <li key={idx} className={`flex items-start gap-2 text-xs ${'text-slate-600 group-hover:text-slate-900'} transition-colors`}>
      ...
      <section className={`py-20 px-6 ${'bg-[#fffaf0]'} relative`}>
         <div className="text-center mb-12 reveal-up">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-slate-900'} mb-3`}>Our Collective <span className={'text-[#b45309]'}>Impact.</span></h2>
            <p className={`text-sm ${'text-slate-500'} font-light`}>What began as an idea transformed into a movement.</p>
         </div>
         
         <div className="grid grid-cols-2 gap-4">
            {METRICS.map((stat, i) => (
               <div key={i} className={`reveal-up p-5 rounded-xl border ${'border-amber-100 bg-white hover:border-amber-300 shadow-sm hover:shadow-xl'} text-center group transition-all duration-500`}>
                  <div className={`w-10 h-10 mx-auto mb-4 rounded-full ${'bg-amber-50 border-amber-200 text-[#b45309] group-hover:bg-[#b45309] group-hover:text-white'} border flex items-center justify-center transition-all duration-500`}>
                     <stat.icon size={18} />
                  </div>
                  <h3 className={`text-2xl font-heading font-black ${'text-slate-900'} mb-1 transition-colors`}>
                     <CountUp end={stat.value} suffix={stat.suffix} />
                  </h3>
                  <p className={`text-[8px] font-black uppercase tracking-[0.2em] ${'text-amber-600/70'} transition-colors`}>{stat.label}</p>
               </div>
            ))}
         </div>
      </section>
      ...
      {/* --- WINNERS SHOWCASE --- */}
      <section className={`py-20 px-6 ${'bg-white border-amber-100'} border-t`}>
         <div className="text-center mb-12 reveal-up">
            <span className={`text-[9px] font-mosaic font-bold uppercase tracking-[0.3em] ${'text-[#b45309]'} block mb-3`}>Prestige</span>
            <h2 className={`text-3xl font-mosaic font-bold ${'text-slate-900'}`}>Champions of Knowledge</h2>
         </div>

         <div className="flex flex-col gap-6">
            {/* 1st Place */}
            <div className="order-1 reveal-up w-full z-10">
               <div className={`relative p-6 rounded-2xl border ${'border-[#b45309] bg-white shadow-2xl'} text-center`}>
                  <div className="absolute -top-5 left-1/2 -translate-x-1/2">
                     <div className={`w-10 h-10 ${'bg-[#b45309] text-white'} rounded-full flex items-center justify-center shadow-lg`}>
                        <Award size={20} className={''} />
                     </div>
                  </div>
                  <div className={`w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden border-2 ${'border-[#b45309]'} mt-3`}>
                     <SmartImage src={WINNERS[1].image} alt={WINNERS[1].name} className="w-full h-full object-cover" />
                  </div>
                  <div className={`text-4xl font-black ${'text-amber-100'} mb-1`}>01</div>
                  <h3 className={`text-xl font-bold ${'text-slate-900'} mb-1`}>{WINNERS[1].name}</h3>
                  <p className={`text-[10px] ${'text-[#b45309]'} uppercase tracking-widest font-bold`}>{WINNERS[1].category}</p>
                  <div className={`mt-4 inline-block px-3 py-1 rounded ${'bg-[#b45309] text-white'} text-xs font-bold font-mono`}>{WINNERS[1].score}</div>
               </div>
            </div>

            {/* 2nd Place */}
            <div className="order-2 reveal-up w-full">
               <div className={`relative p-5 rounded-xl border ${'border-amber-100 bg-[#fffcf5] shadow-lg'} text-center`}>
                  <div className={`w-20 h-20 mx-auto mb-3 rounded-full overflow-hidden border ${'border-slate-300'}`}>
                     <SmartImage src={WINNERS[0].image} alt={WINNERS[0].name} className={`w-full h-full object-cover ${''}`} />
                  </div>
                  <div className={`text-3xl font-black ${'text-slate-300'} mb-1`}>02</div>
                  <h3 className={`text-lg font-bold ${'text-slate-900'} mb-1`}>{WINNERS[0].name}</h3>
                  <p className={`text-[9px] ${'text-[#b45309]'} uppercase tracking-widest`}>{WINNERS[0].category}</p>
                  <div className={`mt-3 inline-block px-2 py-0.5 rounded ${'bg-amber-100 text-amber-800'} text-[10px] font-mono`}>{WINNERS[0].score}</div>
               </div>
            </div>
            ...
            <div className="order-3 reveal-up w-full">
               <div className={`relative p-5 rounded-xl border ${'border-amber-100 bg-[#fffcf5] shadow-lg'} text-center`}>
                  ...
                  <div className={`text-3xl font-black ${'text-slate-300'} mb-1`}>03</div>
                  <h3 className={`text-lg font-bold ${'text-slate-900'} mb-1`}>{WINNERS[2].name}</h3>
                  <p className={`text-[9px] ${'text-[#b45309]'} uppercase tracking-widest`}>{WINNERS[2].category}</p>
                  <div className={`mt-3 inline-block px-2 py-0.5 rounded ${'bg-amber-100 text-amber-800'} text-[10px] font-mono`}>{WINNERS[2].score}</div>
               </div>
            </div>
         </div>
      </section>
      ...
      {/* --- AMBASSADORS --- */}
      <section className={`py-20 px-6 ${'bg-[#fffaf0] border-amber-200'} border-t`}>
         <div className="text-center mb-12 reveal-up">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-slate-900'} mb-3`}>Honoring Our <span className={'text-[#b45309]'}>Ambassadors</span></h2>
         ...
         <div className="flex flex-col gap-4">
            {AMBASSADORS.map((amb, i) => (
               <div key={i} className={`reveal-up p-6 rounded-xl ${'bg-white border-amber-100 hover:border-amber-300 shadow-sm hover:shadow-lg'} border transition-all duration-300 group`}>
                  <div className="flex items-center gap-3 mb-4">
                     <div className={`w-10 h-10 rounded-full ${'bg-amber-50 text-[#b45309] border-amber-200 group-hover:bg-[#b45309] group-hover:text-white'} flex items-center justify-center border transition-colors shrink-0`}>
                        <Users size={16} />
                     </div>
                     <div>
                        <h4 className={`text-base font-bold ${'text-slate-900'}`}>{amb.name}</h4>
                        <p className={`text-[10px] ${'text-slate-500'} uppercase tracking-wide`}>{amb.campus}</p>
                     </div>
                  </div>
                  <p className={`text-xs font-light italic ${'text-slate-600'} leading-relaxed`}>"{amb.quote}"</p>
               </div>
            ))}
         </div>
      </section>
      ...
      <section className={`py-24 px-6 relative overflow-hidden ${'bg-white'}`}>
         ...
         <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] ${'from-amber-100/50'} via-transparent to-transparent opacity-50`} />
         
         <div className="text-center relative z-10 reveal-up">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-slate-900'} mb-4`}>Stay Spiritually <br/><span className={'text-[#b45309]'}>Connected.</span></h2>
            <p className={`text-sm font-light ${'text-slate-500'} mb-8 max-w-sm mx-auto`}>
            ...
            <div className="flex flex-col gap-3 max-w-sm mx-auto">
                <div className="relative">
                    ...
                    <input 
                        ...
                        className={`w-full pl-10 pr-4 py-3 ${'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-[#b45309] focus:ring-[#b45309]'} border rounded-full text-sm focus:outline-none focus:ring-1 transition-all`}
                    />
                </div>
                <button className={`w-full py-3 ${'bg-[#b45309] text-white hover:bg-slate-900 shadow-lg'} font-bold uppercase tracking-widest text-[10px] rounded-full transition-colors`}>
                    Subscribe
                </button>
            </div>
            <p className={`text-[9px] ${'text-slate-400'} mt-4 uppercase tracking-widest`}>We respect your inbox.</p>
         </div>
      </section>
