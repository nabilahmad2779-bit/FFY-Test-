         ...
         <div className="flex flex-col gap-16">
            ...
            <div>
               ...
               <div className={`${isDark ? 'bg-[#0a0a0a]' : 'bg-white'} rounded-sm p-4 border ${isDark ? 'border-[#ad944b]/20' : 'border-[#ad944b]/30'} shadow-2xl mb-8`}>
                 <div className="flex p-1 rounded-sm bg-black/5 mb-6 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:none]">
                   {WINNERS_DATA.map((cat, idx) => (
                     <button
                       key={idx}
                       onClick={() => setActiveCategory(idx)}
                       className={`flex-1 py-2 px-3 rounded-sm text-[10px] font-mosaic font-bold uppercase tracking-wider transition-all whitespace-nowrap ${activeCategory === idx ? 'bg-[#ad944b] text-white shadow-md' : 'text-gray-500 hover:text-[#ad944b]'}`}
                     >
                       {cat.category}
                     </button>
                   ))}
