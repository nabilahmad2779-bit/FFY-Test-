...
export const MosaicStoriesMobile: React.FC<EventPageProps> = ({ onBack, isDark }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
        ...
        gsap.utils.toArray('.segment-card').forEach((card: any) => {
          gsap.to(card, {
            boxShadow: isDark ? "0 0 20px rgba(173,148,75,0.4)" : "0 0 20px rgba(173,148,75,0.3)",
  ...
  return (
    <div 
        ref={containerRef} 
        className={`min-h-screen ${isDark ? 'bg-[#050505] text-[#f9f6f0]' : 'bg-[#fdfcfb] text-[#1a1a1a]'} font-literary selection:bg-[#ad944b] selection:text-black overflow-x-hidden transition-colors duration-500`}
    >
      <div 
        className={`fixed inset-0 pointer-events-none z-[99] opacity-[0.05] mix-blend-overlay ${isDark ? 'invert-0' : 'invert'}`}
      ...
      <section className="relative min-h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden">
         ...
         <div className="relative z-10 text-center flex flex-col items-center pt-20 px-4">
            ...
            <div className="relative py-8 px-2 overflow-visible flex flex-col items-center w-full">
               ...
               <h2 className={`hero-title-sub text-3xl font-mosaic font-bold ${isDark ? 'text-[#f9f6f0]' : 'text-[#1a1a1a]'} uppercase tracking-[0.4em] drop-shadow-xl leading-none mt-2`}>
                  STORIES
               </h2>
            </div>

            <div className="hero-meta mt-12 max-w-[90%]">
               <p className={`text-xl font-literary italic ${isDark ? 'text-[#ad944b]/90' : 'text-[#8e793e]'} font-medium leading-relaxed`}>
                  "{event.tagline}"
               </p>
      ...
      <section className={`reveal-block py-24 px-6 border-t ${isDark ? 'border-[#ad944b]/10' : 'border-[#ad944b]/20'} relative`}>
         ...
         <div className="flex flex-col gap-16 relative z-10">
            ...
            <div className="text-center">
               <h2 className={`text-4xl font-mosaic font-bold ${isDark ? 'text-[#f9f6f0]' : 'text-[#1a1a1a]'} uppercase tracking-tight leading-[1.1] mb-8`}>
                  The Curatorial <br/> <span className="text-[#ad944b] italic font-normal">Synthesis.</span>
               </h2>
            </div>

            <div className="relative w-full max-w-sm mx-auto group">
               <div className={`aspect-[4/5] p-2 ${isDark ? 'bg-[#0a0a0a]' : 'bg-white'} border border-[#ad944b]/30 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden rounded-sm relative`}>
            ...
            <div className={`text-lg font-literary ${isDark ? 'text-[#f9f6f0]/80' : 'text-[#1a1a1a]/80'} leading-relaxed text-justify space-y-8 px-2`}>
      ...
      <section className={`reveal-block py-24 px-6 ${isDark ? 'bg-[#080808]/80' : 'bg-[#f5f5f5]/80'} border-y ${isDark ? 'border-[#ad944b]/10' : 'border-[#ad944b]/20'} backdrop-blur-md`}>
         <div className="text-center mb-16">
            <h2 className="text-[9px] font-mosaic font-bold uppercase tracking-[0.6em] text-[#ad944b] mb-6">The Decagon of Expression</h2>
            <h3 className={`text-4xl font-mosaic font-bold ${isDark ? 'text-[#f9f6f0]' : 'text-[#1a1a1a]'} uppercase tracking-tighter leading-none`}>Event <br/>Segments</h3>
         </div>

         <div className="segments-grid grid grid-cols-2 gap-3">
            {SEGMENTS.map((segment, i) => (
               <div key={i} className={`segment-card group relative p-4 ${isDark ? 'bg-black border-[#ad944b]/10' : 'bg-white border-[#ad944b]/20'} border hover:border-[#ad944b]/50 transition-all duration-500 shadow-xl overflow-hidden rounded-sm flex flex-col items-center text-center`}>
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                     <div className={`w-8 h-8 flex items-center justify-center rounded-lg ${isDark ? 'bg-[#ad944b]/10' : 'bg-[#ad944b]/5'} text-[#ad944b] mb-3 border border-[#ad944b]/20 group-hover:scale-110 group-hover:bg-[#ad944b] ${isDark ? 'group-hover:text-black' : 'group-hover:text-white'} transition-all duration-300`}>
                        <segment.icon size={14} />
                     </div>
                     <h4 className={`text-xs font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase tracking-wider mb-2 leading-tight group-hover:text-[#ad944b] transition-colors`}>
                        {segment.name}
                     </h4>
                     <p className={`text-[9px] font-literary italic ${isDark ? 'text-zinc-500 group-hover:text-zinc-300' : 'text-zinc-600 group-hover:text-zinc-800'} leading-relaxed transition-colors`}>
                        {segment.desc}
                     </p>
                  </div>
               </div>
      ...
      <section className={`reveal-block py-24 px-6 border-t ${isDark ? 'border-[#ad944b]/10 bg-black/40' : 'border-[#ad944b]/20 bg-[#f5f5f5]/50'}`}>
         <div className="flex flex-col gap-16">
            {/* Masters */}
            <div>
               <div className="mb-10 text-center">
                  <h3 className={`text-4xl font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase tracking-tight mb-4 leading-none`}>Masters of the <br/><span className="text-[#ad944b]">Magic.</span></h3>
                  <p className={`text-[9px] font-mosaic font-bold uppercase tracking-[0.3em] ${isDark ? 'text-zinc-500' : 'text-zinc-600'}`}>The High Performance Laureate List</p>
               </div>

               {/* Tab switcher */}
               <div className={`${isDark ? 'bg-[#0a0a0a]' : 'bg-white'} rounded-sm p-4 border ${isDark ? 'border-[#ad944b]/20' : 'border-[#ad944b]/30'} shadow-2xl mb-8`}>
                 <div className={`flex items-center justify-between p-3 rounded-sm ${isDark ? 'bg-black/20 border-[#ad944b]/10' : 'bg-slate-50 border-slate-100'} mb-6 border`}>
                    <button 
                       onClick={() => setActiveCategory(prev => prev === 0 ? WINNERS_DATA.length - 1 : prev - 1)}
                       className={`w-8 h-8 flex items-center justify-center rounded-full bg-[#ad944b]/10 text-[#ad944b] hover:bg-[#ad944b] ${isDark ? 'hover:text-black' : 'hover:text-white'} transition-colors`}
                    >
                       <ChevronLeft size={16} />
                    </button>
                    <div className="text-center">
                       <h4 className={`text-sm font-mosaic font-bold ${isDark ? 'text-white' : 'text-slate-900'} uppercase tracking-widest`}>{WINNERS_DATA[activeCategory].category}</h4>
                       <p className={`text-[9px] font-literary italic ${isDark ? 'text-[#ad944b]' : 'text-slate-500'} mt-0.5`}>Segment {activeCategory + 1} of {WINNERS_DATA.length}</p>
                    </div>
                    <button 
                 ...
                 <div className="space-y-3 min-h-[240px]">
                   {WINNERS_DATA[activeCategory].winners.map((winner, i) => (
                     <div
                       key={i}
                       className={`flex items-center gap-4 p-4 rounded-sm ${isDark ? 'bg-white/[0.02]' : 'bg-[#f8f8f8]'} border ${isDark ? 'border-[#ad944b]/10' : 'border-[#ad944b]/20'}`}
                     >
                       ...
                       <div className="flex-1">
                         <h4 className={`text-sm font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase`}>{winner.name}</h4>
            ...
            {/* Archivists */}
            <div className={`${isDark ? 'bg-[#0a0a0a] border-[#ad944b]/10' : 'bg-white border-[#ad944b]/20'} border p-8 rounded-sm shadow-2xl relative overflow-hidden`}>
               <div className="absolute top-0 right-0 p-6 opacity-[0.03] rotate-12">
                  <Star size={120} className="text-[#ad944b]" />
               </div>
               <div className="relative z-10">
                  <h3 className={`text-3xl font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase tracking-tight mb-10`}>The <span className="text-[#ad944b]">Archivists.</span></h3>
                  ...
                  <div className="relative h-[300px] w-full flex items-center justify-center perspective-[800px] overflow-hidden mt-8">
                     {AMBASSADORS.map((a, i) => {
                        ...
                        return (
                           <div 
                              key={i}
                              className={`absolute w-[200px] p-4 rounded-xl border ${isDark ? 'bg-black/90 border-[#ad944b]/30' : 'bg-white/95 border-[#ad944b]/40'} shadow-2xl backdrop-blur-md transition-all duration-700 ease-out cursor-pointer flex flex-col items-center text-center`}
                              style={{
                           ...
                           >
                              ...
                              <h4 className={`text-sm font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase tracking-widest mb-1`}>{a.name}</h4>
                              <p className={`text-[10px] font-literary italic ${isDark ? 'text-zinc-400' : 'text-zinc-600'} mb-2`}>{a.campus}</p>
      ...
      <section className="reveal-block py-24 px-6">
         <div className="flex flex-col items-center text-center mb-16 gap-6">
            ...
            <h2 className={`text-4xl font-mosaic font-bold ${isDark ? 'text-[#f9f6f0]' : 'text-[#1a1a1a]'} uppercase tracking-tight leading-none`}>
               Gallery of <br/> <span className="text-[#ad944b]">Resonance.</span>
            </h2>
            <p className={`text-lg font-literary ${isDark ? 'text-zinc-500' : 'text-zinc-600'} italic px-4`}>"Every brushstroke is a story, every pixel a memory."</p>
         </div>

         <div className="grid grid-cols-1 gap-6">
            {GALLERY_ARTWORKS.slice(0, 6).map((art, i) => (
              <div key={i} className={`group overflow-hidden rounded-sm border ${isDark ? 'border-[#ad944b]/10 bg-black' : 'border-[#ad944b]/20 bg-white'} flex flex-col shadow-lg`}>
                 ...
                 <div className={`p-6 border-t ${isDark ? 'border-[#ad944b]/10' : 'border-[#ad944b]/20'}`}>
                    <h4 className={`text-lg font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase`}>{art.title}</h4>
      ...
      <section className={`reveal-block py-24 border-t ${isDark ? 'border-[#ad944b]/10 bg-black/40' : 'border-[#ad944b]/20 bg-[#f5f5f5]/50'} overflow-hidden`}>
         <div className="px-6 mb-16 text-center">
            <h3 className={`text-3xl font-mosaic font-bold ${isDark ? 'text-white' : 'text-black'} uppercase tracking-tighter mb-4`}>Allied <span className="text-[#ad944b]">Guilds.</span></h3>
            <p className={`text-[9px] font-mosaic font-bold uppercase tracking-[0.4em] ${isDark ? 'text-zinc-600' : 'text-zinc-500'}`}>The Global Collaborative Architecture</p>
         </div>
         <div className={`relative flex overflow-hidden border-y ${isDark ? 'border-[#ad944b]/5' : 'border-[#ad944b]/10'} py-10`}>
            <div className="flex animate-marquee-legacy gap-12 whitespace-nowrap">
               {[...PARTNER_LOGOS, ...PARTNER_LOGOS, ...PARTNER_LOGOS].map((p, i) => (
                  <div key={i} className="flex flex-col items-center gap-4">
                    <div className={`w-24 h-24 rounded-full border ${isDark ? 'border-[#ad944b]/10 bg-white/[0.02]' : 'border-[#ad944b]/20 bg-white'} flex items-center justify-center p-5 shadow-lg`}>
                       <img src={p.imageUrl} alt={p.name} className={`w-full h-full object-contain ${isDark ? 'grayscale opacity-50' : 'grayscale-0 opacity-80'}`} />
                    </div>
                    <span className={`text-[8px] font-mosaic font-bold uppercase tracking-[0.2em] ${isDark ? 'text-zinc-600' : 'text-zinc-500'}`}>{p.name}</span>
                  </div>
               ))}
            </div>
         </div>
      ...
      <footer className={`py-32 border-t ${isDark ? 'border-[#ad944b]/10 bg-[#050505]' : 'border-[#ad944b]/20 bg-[#fdfcfb]'} flex flex-col items-center justify-center gap-16 relative z-10`}>
         ...
         <div className="text-center relative z-10 space-y-6 px-4">
            <h4 className={`text-6xl font-mosaic font-bold ${isDark ? 'text-zinc-950' : 'text-zinc-200'} uppercase tracking-[0.05em] select-none leading-none`}>MCMXXII</h4>
            <div className="flex flex-col items-center gap-3">
               ...
               <div className="h-px w-16 bg-[#ad944b]/30" />
               <p className={`text-[8px] font-mosaic font-bold ${isDark ? 'text-zinc-800' : 'text-zinc-400'} uppercase tracking-[0.3em] text-center`}>Folio Fig. 2024 • Mosaic Stories Edition</p>
            </div>
         </div>
         <button onClick={onBack} className={`group relative px-12 py-5 border ${isDark ? 'border-[#ad944b]/30' : 'border-[#ad944b]/40'} rounded-sm overflow-hidden transition-all duration-500 shadow-xl bg-transparent`}>
            ...
            <div className={`relative z-10 flex items-center gap-4 font-mosaic font-bold text-[10px] uppercase tracking-[0.4em] ${isDark ? 'text-[#ad944b]' : 'text-[#8e793e]'} group-hover:text-black transition-colors duration-300`}>
