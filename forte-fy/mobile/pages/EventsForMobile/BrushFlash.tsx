...
export const BrushFlashMobile: React.FC<EventPageProps> = ({ isDark, onBack }) => {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ─── 2. INTRO CLIP-PATH & TEXT FILL REVEAL (identical to PC) ───
  ...
  return (
    <div
      ref={containerRef}
      className={`min-h-screen ${isDark ? 'bg-[#050505] text-[#e5e5e5]' : 'bg-[#f8f8f8] text-[#1a1a1a]'} overflow-x-hidden selection:bg-[#ff007f] selection:text-white font-sans transition-colors duration-500`}
    >
      ...
      <div className="fixed top-0 left-0 w-full z-[110] px-6 py-8 flex justify-between items-center pointer-events-none">
        <button
          onClick={onBack}
          className="group flex items-center gap-4 text-[#ff007f]/60 hover:text-[#ff007f] transition-all pointer-events-auto"
        >
          <div className="p-2.5 border-2 border-[#ff007f]/30 group-hover:border-[#ff007f] rounded-full transition-all bg-black/60 backdrop-blur-xl shadow-[0_0_20px_rgba(255,0,127,0.3)]">
            <ArrowLeft size={20} className="text-[#ff007f]" />
          </div>
          <span className="text-[11px] font-black uppercase tracking-[0.5em] translate-y-0.5">Archive</span>
        </button>
      </div>
      ...
      >
        ...
        <div className="max-w-lg mx-auto relative z-10 w-full">
          ...
          <div className="flex flex-col gap-10">
            {/* Text-fill headline — same gradient technique as PC */}
            <h2 className={`text-3xl font-black leading-[1.1] tracking-tighter uppercase text-center ${isDark ? 'text-zinc-800' : 'text-zinc-300'}`}>
              <span className={`text-fill bg-clip-text text-transparent ${isDark ? 'bg-[linear-gradient(to_right,#ffffff_50%,#1a1a1a_50%)]' : 'bg-[linear-gradient(to_right,#1a1a1a_50%,#e5e5e5_50%)]'} bg-[length:200%_100%] bg-[position:100%_0]`}>A celebration of </span>
              <span className={`text-fill bg-clip-text text-transparent ${isDark ? 'bg-[linear-gradient(to_right,#ff007f_50%,#1a1a1a_50%)]' : 'bg-[linear-gradient(to_right,#ff007f_50%,#e5e5e5_50%)]'} bg-[length:200%_100%] bg-[position:100%_0]`}>raw talent </span>
              <span className={`text-fill bg-clip-text text-transparent ${isDark ? 'bg-[linear-gradient(to_right,#ffffff_50%,#1a1a1a_50%)]' : 'bg-[linear-gradient(to_right,#1a1a1a_50%,#e5e5e5_50%)]'} bg-[length:200%_100%] bg-[position:100%_0]`}>and </span>
              <span className={`text-fill bg-clip-text text-transparent ${isDark ? 'bg-[linear-gradient(to_right,#00ffff_50%,#1a1a1a_50%)]' : 'bg-[linear-gradient(to_right,#00ffff_50%,#e5e5e5_50%)]'} bg-[length:200%_100%] bg-[position:100%_0]`}>boundless </span>
              <span className={`text-fill bg-clip-text text-transparent ${isDark ? 'bg-[linear-gradient(to_right,#f4d03f_50%,#1a1a1a_50%)]' : 'bg-[linear-gradient(to_right,#f4d03f_50%,#e5e5e5_50%)]'} bg-[length:200%_100%] bg-[position:100%_0]`}>creativity.</span>
            </h2>
      ...
      >
        ...
        <div className="max-w-lg mx-auto text-center relative z-10">
          ...
          {/* 3 cards stacked vertically on mobile, same metric-item class for GSAP */}
          <div className="flex flex-col gap-6">
            {[
              ...
              return (
                <div
                  key={i}
                  className={`metric-item flex flex-col items-center ${isDark ? 'bg-[#111]' : 'bg-[#f0f0f0]'} p-10 rounded-[2.5rem] border ${isDark ? 'border-white/5' : 'border-black/5'} shadow-2xl`}
                >
                  <h3 className={`text-7xl font-black tracking-tighter mb-4 ${stat.color} drop-shadow-lg`}>
      ...
      >
        ...
        {/* Marquee strip at bottom (identical to PC) */}
        ...
        <div className="disciplines-wrapper flex w-[300vw] h-full">
          {DISCIPLINES.map((disc, idx) => (
            <div
              key={disc.id}
              className="w-[100vw] h-full flex items-center justify-center relative px-6 pt-32 pb-16"
            >
              {/* Panel content mirrors PC layout but stacked for narrow screens */}
              <div className={`flex flex-col items-center justify-center w-full max-w-sm gap-8 z-10 ${idx % 2 !== 0 ? 'flex-col-reverse' : ''}`}>
                <div className="w-full text-center">
                  <h3
                    className="text-5xl font-black tracking-tighter mb-4 uppercase drop-shadow-[0_0_20px_rgba(255,255,255,0.2)]"
                    style={{ color: disc.color }}
                  >
                    {disc.title}
                  </h3>
                  <p className={`text-base ${isDark ? 'text-zinc-300' : 'text-zinc-600'} font-light leading-relaxed`}>
                    {disc.description}
                  </p>
                </div>
                <div
                  className={`w-full ${disc.aspect} overflow-hidden rounded-[2rem] border-2 shadow-2xl relative group`}
                  style={{ borderColor: `${disc.color}80`, boxShadow: `0 0 50px ${disc.accent}` }}
                >
                  <img
                    src={disc.imageUrl}
                    alt={disc.title}
                    className="disc-img w-full h-full object-cover scale-110 group-hover:scale-125 transition-transform duration-1000"
                    referrerPolicy="no-referrer"
                  />
      ...
      >
        <div className="max-w-lg mx-auto">
          ...
          <div className={`${isDark ? 'bg-[#111]' : 'bg-white'} rounded-[2rem] p-4 border ${isDark ? 'border-white/5' : 'border-black/5'} shadow-2xl mb-10`}>
            <div className="flex p-1 rounded-xl bg-black/5 mb-6 overflow-x-auto">
              {WINNERS.map((cat, idx) => (
                <button
                  key={idx}
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              <div
                key={i}
                className={`flex items-center gap-5 ${isDark ? 'bg-[#111]' : 'bg-white'} p-5 rounded-3xl border ${isDark ? 'border-white/5' : 'border-black/5'} shadow-lg relative overflow-hidden`}
              >
                ...
                <div className="text-left relative z-10">
                  <h4 className={`text-base font-black ${isDark ? 'text-white' : 'text-black'} uppercase mb-0.5`}>{leader.name}</h4>
