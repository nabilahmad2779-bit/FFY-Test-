...
export const SpiritualQuestMobile: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const isDark = true;
  ...
  return (
    <div ref={containerRef} className={`min-h-screen ${'bg-[#020202] text-[#f0fdf4] selection:bg-[#D4AF37] selection:text-[#042f2e]'} font-literary overflow-x-hidden`}>
      
      {/* BACK BUTTON */}
      <div className="fixed top-6 left-4 z-50">
        <button onClick={onBack} className={`group flex items-center gap-2 ${'text-[#D4AF37] hover:text-white bg-black/40 border-[#D4AF37]/20 hover:border-[#D4AF37]/50'} transition-colors px-3 py-2 rounded-full backdrop-blur-md border`}>
      ...
      {/* --- HERO SECTION --- */}
      <section className={`relative min-h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden border-b ${'border-[#D4AF37]/10'}`}>
        ...
        <div className="relative z-20 text-center flex flex-col items-center w-full px-4 pt-20 pointer-events-none">
          <div className="mb-6 flex flex-col items-center gap-2 opacity-90">
             <div className={`text-[10px] font-mosaic font-bold uppercase tracking-[0.4em] ${'text-[#D4AF37]'} drop-shadow-md`}>Forte-FY Presents</div>
             <div className={`w-12 h-[1px] ${'bg-[#D4AF37]'}`} />
          </div>
          <div className="relative mb-6 w-full">
             <h1 className={`text-[18vw] font-mosaic font-black leading-[0.9] tracking-normal text-transparent bg-clip-text ${'bg-gradient-to-b from-[#F9E9A8] via-[#D4AF37] to-[#B8860B] drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)]'}`}>
                The <br/> Spiritual <br/> Quest
             </h1>
          </div>
          <p className={`text-sm font-literary italic ${'text-[#D4AF37] drop-shadow-md'} tracking-wider px-2`}>
      ...
      <section className={`py-20 px-6 relative z-10 ${'bg-[#020202]'}`}>
         <div className="reveal-up text-center space-y-8">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-white'} leading-tight`}>
               <span className={`${'text-[#D4AF37]'} block mb-1`}>A Journey</span> of Knowledge & Reflection.
            </h2>
            
            <div className={`relative group/text-container space-y-6 text-base font-light ${'text-zinc-400'} leading-relaxed max-w-sm mx-auto`}>
               <div className={`absolute -left-6 -top-6 ${'text-[#D4AF37]/10'} animate-pulse`}>
                  <Sparkles size={40} />
               </div>

               <p className={`transition-all duration-500 ${'hover:text-white'} cursor-default`}>
      ...
      <section className={`py-20 px-4 ${'bg-[#050505]'} relative overflow-hidden`}>
         ...
         <div className="relative z-10">
            <div className="text-center mb-12 reveal-up">
               <span className={`text-[9px] font-mosaic font-bold uppercase tracking-[0.3em] ${'text-[#D4AF37]'} block mb-3`}>Daily Guidance</span>
               <h2 className={`text-3xl font-mosaic font-bold ${'text-white'} mb-4`}>Ramadan Reflection Calendar</h2>
               <p className={`text-sm ${'text-zinc-400'} font-light px-4`}>
            ...
            <div className="flex overflow-x-auto pb-8 gap-4 snap-x snap-mandatory no-scrollbar px-2" style={{ scrollBehavior: 'smooth' }}>
               {CALENDAR_DAYS.map((day, i) => (
                  <div key={i} className={`reveal-up snap-center shrink-0 w-[260px] p-6 rounded-2xl ${'bg-[#0a0a0a] border-[#D4AF37]/20 hover:border-[#D4AF37]'} border transition-all duration-500 group relative overflow-hidden flex flex-col h-[320px]`}>
                     
                     <div className={`absolute top-0 right-0 p-3 ${'opacity-5 group-hover:opacity-20'} transition-opacity`}>
                        <Moon size={80} className={'text-[#D4AF37]'} />
                     </div>
                     
                     <div className="relative z-10 flex flex-col h-full">
                        <div className="flex justify-between items-start mb-4">
                           <div className={`inline-block px-3 py-1 rounded-full border ${'border-[#D4AF37]/30 bg-[#D4AF37]/5 text-[#D4AF37]'} text-[10px] font-bold uppercase tracking-widest`}>
                              Day {day.day}
                           </div>
                           {/* Category Marker */}
                           <div className={`text-[8px] font-black uppercase tracking-widest ${'text-zinc-600 group-hover:text-[#D4AF37]'} transition-colors`}>
                              {day.day <= 10 ? 'Mercy' : day.day <= 20 ? 'Forgiveness' : 'Salvation'}
                           </div>
                        </div>

                        <h3 className={`text-xl font-mosaic font-bold ${'text-white group-hover:text-[#D4AF37]'} mb-4 leading-tight transition-colors`}>
                           {day.title}
                        </h3>
                        
                        <ul className="space-y-3 mt-auto">
                           {day.tasks.map((task, idx) => (
                              <li key={idx} className={`flex items-start gap-2 text-xs ${'text-zinc-400 group-hover:text-zinc-200'} transition-colors`}>
      ...
      {/* --- IMPACT METRICS --- */}
      <section className={`py-20 px-6 ${'bg-[#020202]'} relative`}>
         <div className="text-center mb-12 reveal-up">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-white'} mb-3`}>Our Collective <span className={'text-[#D4AF37]'}>Impact.</span></h2>
            <p className={`text-sm ${'text-zinc-400'} font-light`}>What began as an idea transformed into a movement.</p>
         </div>
         
         <div className="grid grid-cols-2 gap-4">
            {METRICS.map((stat, i) => (
               <div key={i} className={`reveal-up p-5 rounded-xl border ${'border-[#D4AF37]/10 bg-[#080808] hover:border-[#D4AF37]/40'} text-center group transition-all duration-500`}>
                  <div className={`w-10 h-10 mx-auto mb-4 rounded-full ${'bg-[#D4AF37]/5 border-[#D4AF37]/20 text-[#D4AF37] group-hover:bg-[#D4AF37] group-hover:text-black'} border flex items-center justify-center transition-all duration-500`}>
                     <stat.icon size={18} />
                  </div>
                  <h3 className={`text-2xl font-heading font-black ${'text-white group-hover:text-[#D4AF37]'} mb-1 transition-colors`}>
                     <CountUp end={stat.value} suffix={stat.suffix} />
                  </h3>
                  <p className={`text-[8px] font-black uppercase tracking-[0.2em] ${'text-zinc-500 group-hover:text-zinc-300'} transition-colors`}>{stat.label}</p>
               </div>
            ))}
         </div>
      </section>
      ...
      {/* --- WINNERS SHOWCASE --- */}
      <section className={`py-20 px-6 ${'bg-[#050505] border-[#D4AF37]/10'} border-t`}>
         <div className="text-center mb-12 reveal-up">
            <span className={`text-[9px] font-mosaic font-bold uppercase tracking-[0.3em] ${'text-[#D4AF37]'} block mb-3`}>Prestige</span>
            <h2 className={`text-3xl font-mosaic font-bold ${'text-white'}`}>Champions of Knowledge</h2>
         </div>

         <div className="flex flex-col gap-6">
            {/* 1st Place */}
            <div className="order-1 reveal-up w-full z-10">
               <div className={`relative p-6 rounded-2xl border ${'border-[#D4AF37] bg-[#0c0c0c] shadow-[0_0_30px_rgba(212,175,55,0.15)]'} text-center`}>
                  <div className="absolute -top-5 left-1/2 -translate-x-1/2">
                     <div className={`w-10 h-10 ${'bg-[#D4AF37]'} rounded-full flex items-center justify-center shadow-lg`}>
                        <Award size={20} className={'text-black'} />
                     </div>
                  </div>
                  <div className={`w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden border-2 ${'border-[#D4AF37]'} mt-3`}>
                     <SmartImage src={WINNERS[1].image} alt={WINNERS[1].name} className="w-full h-full object-cover" />
                  </div>
                  <div className={`text-4xl font-black ${'text-[#D4AF37]/20'} mb-1`}>01</div>
                  <h3 className={`text-xl font-bold ${'text-white'} mb-1`}>{WINNERS[1].name}</h3>
                  <p className={`text-[10px] ${'text-[#D4AF37]'} uppercase tracking-widest font-bold`}>{WINNERS[1].category}</p>
                  <div className={`mt-4 inline-block px-3 py-1 rounded ${'bg-[#D4AF37] text-black'} text-xs font-bold font-mono`}>{WINNERS[1].score}</div>
               </div>
            </div>

            {/* 2nd Place */}
            <div className="order-2 reveal-up w-full">
               <div className={`relative p-5 rounded-xl border ${'border-[#D4AF37]/20 bg-[#0a0a0a]'} text-center`}>
                  <div className={`w-20 h-20 mx-auto mb-3 rounded-full overflow-hidden border ${'border-zinc-700'}`}>
                     <SmartImage src={WINNERS[0].image} alt={WINNERS[0].name} className={`w-full h-full object-cover ${'grayscale group-hover:grayscale-0'}`} />
                  </div>
                  <div className={`text-3xl font-black ${'text-zinc-700'} mb-1`}>02</div>
                  <h3 className={`text-lg font-bold ${'text-white'} mb-1`}>{WINNERS[0].name}</h3>
                  <p className={`text-[9px] ${'text-[#D4AF37]'} uppercase tracking-widest`}>{WINNERS[0].category}</p>
                  <div className={`mt-3 inline-block px-2 py-0.5 rounded ${'bg-white/5'} text-[10px] font-mono`}>{WINNERS[0].score}</div>
               </div>
            </div>
            ...
            <div className="order-3 reveal-up w-full">
               <div className={`relative p-5 rounded-xl border ${'border-[#D4AF37]/20 bg-[#0a0a0a]'} text-center`}>
                  ...
                  <div className={`text-3xl font-black ${'text-zinc-700'} mb-1`}>03</div>
                  <h3 className={`text-lg font-bold ${'text-white'} mb-1`}>{WINNERS[2].name}</h3>
                  <p className={`text-[9px] ${'text-[#D4AF37]'} uppercase tracking-widest`}>{WINNERS[2].category}</p>
                  <div className={`mt-3 inline-block px-2 py-0.5 rounded ${'bg-white/5'} text-[10px] font-mono`}>{WINNERS[2].score}</div>
               </div>
            </div>
         </div>
      </section>
      ...
      {/* --- AMBASSADORS --- */}
      <section className={`py-20 px-6 ${'bg-[#020202] border-[#D4AF37]/10'} border-t`}>
         <div className="text-center mb-12 reveal-up">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-white'} mb-3`}>Honoring Our <span className={'text-[#D4AF37]'}>Ambassadors</span></h2>
         ...
         <div className="flex flex-col gap-4">
            {AMBASSADORS.map((amb, i) => (
               <div key={i} className={`reveal-up p-6 rounded-xl ${'bg-[#080808] border-white/5 hover:border-[#D4AF37]/30'} border transition-all duration-300 group`}>
                  <div className="flex items-center gap-3 mb-4">
                     <div className={`w-10 h-10 rounded-full ${'bg-[#D4AF37]/10 text-[#D4AF37] border-[#D4AF37]/20 group-hover:bg-[#D4AF37] group-hover:text-black'} flex items-center justify-center border transition-colors shrink-0`}>
                        <Users size={16} />
                     </div>
                     <div>
                        <h4 className={`text-base font-bold ${'text-white'}`}>{amb.name}</h4>
                        <p className={`text-[10px] ${'text-zinc-500'} uppercase tracking-wide`}>{amb.campus}</p>
                     </div>
                  </div>
                  <p className={`text-xs font-light italic ${'text-zinc-300'} leading-relaxed`}>"{amb.quote}"</p>
               </div>
            ))}
         </div>
      </section>
      ...
      <section className={`py-24 px-6 relative overflow-hidden ${''}`}>
         ...
         <div className="text-center relative z-10 reveal-up">
            <h2 className={`text-3xl font-mosaic font-bold ${'text-white'} mb-4`}>Stay Spiritually <br/><span className={'text-[#D4AF37]'}>Connected.</span></h2>
            <p className={`text-sm font-light ${'text-zinc-400'} mb-8 max-w-sm mx-auto`}>
            ...
            <div className="flex flex-col gap-3 max-w-sm mx-auto">
                <div className="relative">
                    ...
                    <input 
                        ...
                        className={`w-full pl-10 pr-4 py-3 ${'bg-[#0a0a0a] border-[#D4AF37]/30 text-white placeholder-zinc-500 focus:border-[#D4AF37] focus:ring-[#D4AF37]'} border rounded-full text-sm focus:outline-none focus:ring-1 transition-all`}
                    />
                </div>
                <button className={`w-full py-3 ${'bg-[#D4AF37] text-black hover:bg-white shadow-[0_0_15px_rgba(212,175,55,0.3)]'} font-bold uppercase tracking-widest text-[10px] rounded-full transition-colors`}>
                    Subscribe
                </button>
            </div>
            <p className={`text-[9px] ${'text-zinc-600'} mt-4 uppercase tracking-widest`}>We respect your inbox.</p>
         </div>
      </section>
