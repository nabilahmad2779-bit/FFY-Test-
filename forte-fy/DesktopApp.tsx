          ...
          <div className="flex animate-marquee whitespace-nowrap gap-16 md:gap-32 py-8 items-start">
             {partners.map((partner, i) => (
                  <div key={i} className="flex flex-col items-center gap-6 md:gap-10 transition-all duration-700 group shrink-0">
                     <div className={`w-24 h-24 md:w-48 md:h-48 rounded-full overflow-hidden border-2 p-3 md:p-6 transition-all duration-500 shadow-xl flex items-center justify-center ${isDark ? 'bg-zinc-900 border-white/20 shadow-cyan-500/10' : 'bg-slate-50 border-slate-200 shadow-slate-200'}`}>
...
const HomeView: React.FC<{ navigate: (path: string) => void; handleDownload: () => void; isDark: boolean }> = ({ navigate, handleDownload, isDark }) => {
  return (
    <>
      <section id={NavSection.Home} className="relative min-h-[85vh] flex flex-col items-center justify-center pt-16 md:pt-20 px-5 overflow-hidden group/hero">
        <div className="absolute inset-0 z-0 overflow-hidden">
           <img src={HERO_IMAGE_URL} className={`w-full h-full object-cover object-center transition-all duration-[3000ms] group-hover/hero:scale-105 ${isDark ? 'grayscale brightness-[0.4]' : 'brightness-100'}`} alt="Hero Backdrop" />
           <div className={`absolute inset-0 bg-gradient-to-b transition-all duration-1000 ${isDark ? 'from-zinc-950 via-transparent to-zinc-950 opacity-80' : 'from-slate-100/80 via-slate-100/20 to-slate-100/80 opacity-100'}`} />
        </div>
        <div className="max-w-6xl mx-auto w-full relative z-10 flex flex-col items-center text-center">
          <ScrollReveal>
            ...
            <h1 className="text-4xl sm:text-6xl md:text-8xl lg:text-[6rem] font-heading font-black uppercase leading-[0.9] mb-6 md:mb-8 tracking-tighter w-full px-2 py-4">
              <span className={`italic ${isDark ? 'text-white' : 'text-slate-900'}`}>Architecting</span> <br />
              <span className={`italic ${isDark ? 'text-[#00f7ff] drop-shadow-sm' : 'text-[#0066cc]'}`}>Tomorrow.</span>
            </h1>
            
            <p className={`text-sm md:text-xl max-w-2xl mb-8 md:mb-12 leading-relaxed text-center ${isDark ? 'font-light text-zinc-300' : 'font-medium text-slate-700'} px-2 mx-auto`}>
              Established May 26, 2022. We manufacture a generation of accomplished individuals through rigorous skill elevation.
            </p>
...
const DesktopApp: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen text-center selection:bg-[#00f7ff] w-full flex flex-col transition-colors duration-500 ${isDark ? 'bg-[#030303] text-white selection:text-black' : 'bg-slate-50 text-slate-900 selection:bg-[#0066cc] selection:text-white'}`}>
      ...
      {/* Global Nav */}
      {currentView !== 'detail' && (
        <nav className={`fixed top-0 w-full z-50 transition-all duration-500 h-16 md:h-20 flex items-center px-4 md:px-0 ${
          scrollDirection === 'down' ? '-translate-y-full' : 'translate-y-0'
        } ${scrolled ? (isDark ? 'bg-black/95 backdrop-blur-xl border-b border-[#00f7ff]/10' : 'bg-white/95 backdrop-blur-xl border-b border-slate-200') : 'bg-transparent'}`}>
          <div className="max-w-7xl mx-auto px-4 lg:px-10 w-full flex justify-between items-center">
            <div className="flex items-center gap-2 md:gap-3 cursor-pointer group" onClick={() => navigate('/')}>
              <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-transform group-hover:rotate-180 duration-1000 overflow-hidden`}>
                <img src="https://i.postimg.cc/8P4w3j9V/forte-fy-logo.png" alt="Forte-FY Logo" className="w-full h-full object-cover" />
              </div>
