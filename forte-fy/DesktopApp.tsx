...
const InteractiveArchive: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  return (
    <section id={NavSection.Laurels} className={`relative py-16 md:py-36 overflow-hidden border-t transition-colors duration-500 ${isDark ? 'bg-[#030707] border-white/5' : 'bg-slate-100 border-slate-300'}`}>
                ...
                {COLLABORATIONS.map((collab, i) => (
                  <ScrollReveal key={i} delay={i * 80} className="group relative flex flex-col md:flex-row items-start">
                    ...
                    <div className="relative z-20 shrink-0 md:mt-2.5 hidden sm:flex">
                       <div className={`w-10 h-10 md:w-11 md:h-11 rounded-full border flex items-center justify-center transition-all duration-500 ${isDark ? 'bg-[#050909] border-white/10 group-hover:border-[#00f7ff]/50' : 'bg-white border-slate-300 group-hover:border-[#0066cc] shadow-md'}`}>
...
const NeuralNexusCommand: React.FC<NeuralNexusCommandProps> = ({ onNav, onDownload, isDark }) => {
  ...
  return (
    <div className={`w-full py-16 md:py-24 px-4 md:px-8 relative overflow-hidden min-h-screen flex flex-col items-center justify-center transition-colors duration-500 ${isDark ? 'bg-[#030303]' : 'bg-slate-100'}`}>
      ...
      <div className="max-w-[1400px] mx-auto relative z-10 w-full">
          ...
          {FORTE_EVENTS.map((event) => (
            <button
              ...
              className={`flex-none snap-start md:flex-1 min-w-[150px] md:min-w-[200px] px-5 py-3 md:px-8 md:py-6 rounded-lg md:rounded-2xl transition-all duration-700 relative overflow-hidden group border ${
                selectedId === event.id 
                ? (isDark ? 'bg-[#0a0a0a] border-[#00f7ff]/60 text-white shadow-lg' : 'bg-[#0066cc] border-[#0066cc] text-white shadow-lg')
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 md:gap-6 items-stretch">
          <div className="lg:col-span-5 flex flex-col gap-5 md:gap-6 order-2 lg:order-1">
            <div 
              className={`rounded-[1.5rem] md:rounded-[2.5rem] border p-8 md:p-12 flex flex-col items-center justify-center relative overflow-hidden group shadow-sm hover:shadow-xl cursor-help flex-1 min-h-[220px] transition-all duration-500 ${isDark ? 'bg-[#080808] border-white/10' : 'bg-white border-slate-300'}`}
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-6">
               <div 
                 className={`rounded-[1.2rem] md:rounded-[2rem] border p-5 md:p-8 flex flex-col items-center justify-center text-center group transition-all cursor-help hover:shadow-lg ${isDark ? 'bg-[#080808] border-white/10 hover:border-[#00f7ff]/30 hover:bg-[#0a0a0a]' : 'bg-white border-slate-300 hover:border-[#0066cc]/50'}`}
          ...
          <div className={`lg:col-span-7 rounded-[1.5rem] md:rounded-[2.5rem] border p-6 md:p-12 shadow-sm hover:shadow-xl relative overflow-hidden flex flex-col h-full order-1 lg:order-2 transition-colors ${isDark ? 'bg-[#080808] border-white/10' : 'bg-white border-slate-300'}`}>
            ...
            <div className={`flex flex-col md:flex-row items-center gap-5 md:gap-8 pt-5 md:pt-8 border-t mt-auto ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
               <div className={`p-5 md:p-8 rounded-[1.2rem] md:rounded-[2rem] border flex flex-col items-center justify-center w-full md:w-auto md:min-w-[200px] shadow-sm relative group overflow-hidden ${isDark ? 'bg-[#111] border-white/10' : 'bg-slate-900 text-white border-slate-900'}`}>
...
const DepartmentsSection: React.FC<{ navigate: (path: string) => void; isDark: boolean }> = ({ navigate, isDark }) => {
  return (
    <section id={NavSection.Architecture} className={`py-16 md:py-32 px-5 md:px-6 relative overflow-hidden border-t transition-colors duration-500 ${isDark ? 'bg-[#050505] border-white/5' : 'bg-slate-100 border-slate-300'}`}>
...
const PartnersSection: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  ...
  return (
    <section className={`py-16 md:py-32 overflow-hidden border-t relative transition-colors duration-500 ${isDark ? 'bg-black border-white/5' : 'bg-white border-slate-300'}`}>
    ...
    <>
      <section id={NavSection.Home} className="relative min-h-screen flex flex-col items-center justify-center pt-16 md:pt-20 px-5 overflow-hidden group/hero">
        <div className="absolute inset-0 z-0 overflow-hidden">
           ...
           <div className={`absolute inset-0 bg-gradient-to-b transition-all duration-1000 ${isDark ? 'from-[#030303] via-transparent to-[#030303] opacity-60' : 'from-slate-100/80 via-slate-100/20 to-slate-100/80 opacity-100'}`} />
        </div>
      ...
      <section className={`py-16 md:py-32 px-5 relative overflow-hidden border-t transition-colors duration-500 ${isDark ? 'bg-black border-[#00f7ff]/10' : 'bg-white border-slate-300'}`}>
      ...
      <section id={NavSection.Contact} className={`py-16 md:py-32 px-5 relative overflow-hidden border-t transition-colors duration-500 ${isDark ? 'bg-black border-white/5' : 'bg-slate-100 border-slate-300'}`}>
...
const DesktopApp: React.FC = () => {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'events': return <EventsPage isDark={isDark} navigate={navigate} />;
