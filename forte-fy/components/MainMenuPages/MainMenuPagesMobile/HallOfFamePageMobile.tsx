...
import { HallOfFamePageMobileLight } from './MobileMenuLight/HallOfFamePageMobileLight.tsx';

export const HallOfFamePageMobile: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  if (!isDark) {
    return <HallOfFamePageMobileLight />;
  }

  return (
    <div className="pb-24 pt-24 px-6 min-h-screen bg-black text-white animate-fade-in">
      ...
      <div className="space-y-20">
        {HALL_OF_FAME.map((member, i) => (
          <ScrollReveal key={i} delay={i * 100} className="flex flex-col items-center text-center">
             <div className="w-44 h-44 rounded-full overflow-hidden border-2 p-1.5 mb-8 border-cyan-500/30">
             ...
             <div className="relative px-4">
                ...
                <p className="text-base italic leading-relaxed text-zinc-400">"{member.impact}"</p>
             </div>
          </ScrollReveal>
        ))}
      </div>
