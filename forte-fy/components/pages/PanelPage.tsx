...
export const PanelPage: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#030303] text-[#F5F5F5] transition-colors duration-700 overflow-x-hidden relative">
      {/* Dynamic Floating Background Elements */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-[20%] left-[10%] w-[300px] h-[300px] rounded-full blur-[100px] opacity-20 bg-[#D4AF37] animate-pulse" style={{ animationDuration: '8s' }} />
        <div className="absolute bottom-[20%] right-[10%] w-[400px] h-[400px] rounded-full blur-[120px] opacity-20 bg-white animate-pulse" style={{ animationDuration: '12s', animationDelay: '2s' }} />
      </div>
      ...
      <section className="hero-section min-h-screen w-full flex flex-col items-center justify-center px-6 pt-32 pb-20 text-center relative z-10 overflow-hidden">
         ...
         <h1 className="flex flex-col items-center justify-center w-full relative z-10">
           ...
           <AnimatedText text="BODY" className="font-cormorant font-bold italic text-4xl md:text-6xl lg:text-7xl xl:text-[8rem] leading-[1.1] tracking-widest text-[#D4AF37] mt-2 md:mt-4" />
         </h1>
         
         <div className="hero-fade mt-12 max-w-2xl mx-auto relative z-10">
            <p className="font-outfit text-sm md:text-base uppercase tracking-[0.3em] text-zinc-400 leading-relaxed font-medium">
        ...
        {MEMBERS.map((member, index) => (
          <div key={index} className={`member-row flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-12 md:gap-20 lg:gap-32`}>
            ...
            <div className="w-full md:w-1/2 flex flex-col justify-center text-center md:text-left relative z-10">
               <p className="member-text font-outfit uppercase tracking-[0.2em] text-[#D4AF37] text-xs md:text-sm mb-6 font-medium">
                 {member.role}
               </p>
               <h2 className="member-text font-cormorant font-bold text-4xl md:text-5xl lg:text-6xl xl:text-7xl mb-8 leading-tight">
                 {member.name}
               </h2>
               <p className="member-text font-outfit font-light text-zinc-400 leading-relaxed text-base md:text-lg max-w-xl mx-auto md:mx-0">
                 {member.bio}
               </p>
            </div>
      ...
      <section className="py-32 w-full flex items-center justify-center">
         <div className="text-center hero-fade">
            <div className="w-1 h-16 mx-auto text-[#D4AF37] mb-8 opacity-50" style={{ backgroundColor: 'currentColor' }} />
         </div>
      </section>
