...
export const PanelPage: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  ...
  // Custom Themed Color Palette
  const bg = isDark ? "bg-[#030303]" : "bg-[#FDFCF8]";
  const text = isDark ? "text-[#F5F5F5]" : "text-[#1A1A1A]";
  const accentText = isDark ? "text-[#D4AF37]" : "text-[#B8860B]";
  const mutedText = isDark ? "text-zinc-400" : "text-zinc-600";

  return (
    <div ref={containerRef} className={`min-h-screen ${bg} ${text} transition-colors duration-700 overflow-x-hidden relative`}>
      {/* Dynamic Floating Background Elements */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className={`absolute top-[20%] left-[10%] w-[300px] h-[300px] rounded-full blur-[100px] opacity-20 ${isDark ? 'bg-[#D4AF37]' : 'bg-[#B8860B]'} animate-pulse`} style={{ animationDuration: '8s' }} />
        <div className={`absolute bottom-[20%] right-[10%] w-[400px] h-[400px] rounded-full blur-[120px] opacity-20 ${isDark ? 'bg-white' : 'bg-black'} animate-pulse`} style={{ animationDuration: '12s', animationDelay: '2s' }} />
      </div>
      ...
      <section className="hero-section min-h-screen w-full flex flex-col items-center justify-center px-6 pt-32 pb-20 text-center relative z-10 overflow-hidden">
         ...
         <div className="absolute inset-0 z-0 overflow-hidden">
           ...
           />
           <div className={`absolute inset-0 bg-gradient-to-b ${isDark ? 'from-[#030303]/90 via-[#030303]/60 to-[#030303]' : 'from-[#FDFCF8]/90 via-[#FDFCF8]/60 to-[#FDFCF8]'}`} />
         </div>
         ...
         <h1 className="flex flex-col items-center justify-center w-full relative z-10">
           ...
           <AnimatedText text="BODY" className={`font-cormorant font-bold italic text-4xl md:text-6xl lg:text-7xl xl:text-[8rem] leading-[1.1] tracking-widest ${accentText} mt-2 md:mt-4`} />
         </h1>
         
         <div className="hero-fade mt-12 max-w-2xl mx-auto relative z-10">
            <p className={`font-outfit text-sm md:text-base uppercase tracking-[0.3em] ${mutedText} leading-relaxed font-medium`}>
        ...
        {MEMBERS.map((member, index) => (
          <div key={index} className={`member-row flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-12 md:gap-20 lg:gap-32`}>
            ...
            <div className="w-full md:w-1/2 flex flex-col justify-center text-center md:text-left relative z-10">
               <p className={`member-text font-outfit uppercase tracking-[0.2em] ${accentText} text-xs md:text-sm mb-6 font-medium`}>
                 {member.role}
               </p>
               <h2 className="member-text font-cormorant font-bold text-4xl md:text-5xl lg:text-6xl xl:text-7xl mb-8 leading-tight">
                 {member.name}
               </h2>
               <p className={`member-text font-outfit font-light ${mutedText} leading-relaxed text-base md:text-lg max-w-xl mx-auto md:mx-0`}>
                 {member.bio}
               </p>
            </div>
      ...
      <section className="py-32 w-full flex items-center justify-center">
         <div className="text-center hero-fade">
            <div className={`w-1 h-16 mx-auto ${accentText} mb-8 opacity-50`} style={{ backgroundColor: 'currentColor' }} />
         </div>
      </section>
