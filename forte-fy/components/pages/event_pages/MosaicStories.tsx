  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
        ...
        gsap.utils.toArray('.segment-card').forEach((card: any) => {
          gsap.to(card, {
            boxShadow: "0 0 40px rgba(173,148,75,0.4)",
    ...
    >
      ...
      <section className="reveal-block py-48 px-6 bg-[#080808]/50 border-y border-[#ad944b]/10 backdrop-blur-sm">
         <div className="max-w-7xl mx-auto">
            ...
            <div className="segments-grid grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
               {SEGMENTS.map((segment, i) => (
                  <div key={i} className="segment-card group relative p-6 bg-black border border-[#ad944b]/10 hover:border-[#ad944b]/50 transition-all duration-700 shadow-2xl overflow-hidden rounded-sm cursor-default flex flex-col items-center text-center h-[220px]">
                     <div className="absolute -right-8 -bottom-8 opacity-[0.03] group-hover:opacity-[0.1] transition-opacity duration-1000 rotate-12">
                        <segment.icon size={120} className="text-[#ad944b]" />
                     </div>
                     <div className="relative z-10 flex flex-col items-center h-full">
                        <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-[#ad944b]/10 text-[#ad944b] mb-4 border border-[#ad944b]/20 group-hover:scale-110 group-hover:bg-[#ad944b] group-hover:text-black transition-all duration-500">
                           <segment.icon size={20} />
                        </div>
                        <h4 className="text-sm font-mosaic font-bold text-white uppercase tracking-wider mb-2 leading-tight group-hover:text-[#ad944b] transition-colors">
                           {segment.name}
                        </h4>
                        <p className="text-[10px] font-literary italic text-zinc-500 leading-relaxed group-hover:text-zinc-200 transition-colors mb-4">
                           {segment.desc}
                        </p>
                        <div className="mt-auto h-[1px] w-8 bg-[#ad944b]/40 group-hover:w-full transition-all duration-1000" />
                     </div>
                  </div>
               ))}
            </div>
      ...
      <section className="reveal-block py-48 px-6 border-b border-[#ad944b]/10 bg-black/40">
         <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-24 items-start">
            <div className="lg:col-span-7">
               <div className="mb-16">
                  <h3 className="text-5xl md:text-7xl font-mosaic font-bold text-white uppercase tracking-tight mb-6 leading-none">Masters of the <span className="text-[#ad944b]">Magic.</span></h3>
               ...
               {/* Tab switcher */}
               <div className="bg-[#0a0a0a] rounded-sm p-6 border border-[#ad944b]/20 shadow-2xl mb-10">
                 <div className="flex items-center justify-between p-4 rounded-sm bg-black/20 mb-8 border border-[#ad944b]/10">
                    <button 
                       onClick={() => setActiveCategory(prev => prev === 0 ? WINNERS_DATA.length - 1 : prev - 1)}
                       className="w-10 h-10 flex items-center justify-center rounded-full bg-[#ad944b]/10 text-[#ad944b] hover:bg-[#ad944b] hover:text-black transition-colors"
                    >
                       <ChevronLeft size={20} />
                    </button>
                    <div className="text-center">
                       <h4 className="text-lg md:text-xl font-mosaic font-bold text-white uppercase tracking-widest">{WINNERS_DATA[activeCategory].category}</h4>
                       <p className="text-[10px] font-literary italic text-[#ad944b] mt-1">Segment {activeCategory + 1} of {WINNERS_DATA.length}</p>
                    </div>
                    <button 
                 ...
                 <div className="space-y-4 min-h-[300px]">
                   {WINNERS_DATA[activeCategory].winners.map((winner, i) => (
                     <div
                       key={i}
                       className="flex items-center gap-6 p-6 rounded-sm bg-white/[0.02] border border-[#ad944b]/10 hover:border-[#ad944b]/30 transition-all duration-300"
                     >
                       <div className="relative">
                         ...
                         <div className="absolute -bottom-2 -right-2 w-7 h-7 rounded-full bg-black text-white flex items-center justify-center text-[10px] font-bold border-2 border-[#ad944b]">
                           {i + 1}
                         </div>
                       </div>
                       <div className="flex-1">
                         <h4 className="text-lg font-mosaic font-bold text-white uppercase">{winner.name}</h4>
            ...
            <div className="lg:col-span-5 bg-[#0a0a0a] border border-[#ad944b]/10 p-12 md:p-12 rounded-sm shadow-[0_50px_100px_rgba(0,0,0,1)] relative overflow-hidden">
                  ...
                  <div className="relative h-[400px] w-full flex items-center justify-center perspective-[1000px] overflow-hidden">
                     {AMBASSADORS.map((a, i) => {
                        ...
                        return (
                           <div 
                              key={i}
                              className="absolute w-[240px] p-6 rounded-xl border bg-black/80 border-[#ad944b]/30 shadow-2xl backdrop-blur-md transition-all duration-700 ease-out cursor-pointer flex flex-col items-center text-center"
                              style={{
                           ...
                           >
                              <div className="w-24 h-24 rounded-full border-2 border-[#ad944b] overflow-hidden mb-4 shadow-[0_0_15px_rgba(173,148,75,0.5)]">
                                 <img src={a.img} alt={a.name} className="w-full h-full object-cover" />
                              </div>
                              <h4 className="text-xl font-mosaic font-bold text-white uppercase tracking-widest mb-1">{a.name}</h4>
                              <p className="text-xs font-literary italic text-zinc-400 mb-3">{a.campus}</p>
