...
export const BrushFlashLight: React.FC<EventPageProps> = () => {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#fafafa] text-[#111111] font-sans selection:bg-black selection:text-white">
      ...
      <section className="hero-container relative w-full h-[90vh] md:h-screen overflow-hidden px-4 md:px-8 pt-4 md:pt-8">
         <div className="relative w-full h-full rounded-[2rem] md:rounded-[3rem] overflow-hidden">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
               <h1 className="text-[15vw] md:text-[12vw] font-light tracking-tighter text-black leading-none mix-blend-overlay">
                  BRUSH
               </h1>
               <h1 className="text-[15vw] md:text-[12vw] font-light tracking-tighter text-black leading-none mix-blend-overlay">
                  & FLASH
            ...
            <div className="absolute bottom-8 left-8 md:bottom-12 md:left-12">
               <p className="text-sm md:text-base font-light uppercase tracking-widest text-black/80">Exhibition {event.year}</p>
            </div>
         </div>
      </section>
      ...
      <section className="py-32 px-6 md:px-24 max-w-7xl mx-auto">
         <div className="fade-up max-w-4xl">
            <h2 className="text-3xl md:text-5xl font-light leading-tight tracking-tight text-black mb-12">
         ...
         <div className="mt-32 grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-8 border-t border-slate-200 pt-16">
            {[
               ...
               <div key={i} className="fade-up flex flex-col items-start">
                  <Magnetic>
                     <h3 className="text-6xl md:text-8xl font-light tracking-tighter text-black mb-4 transition-colors hover:text-[#00c8c8]">
                        <AnimatedNumber value={stat.val} />
                     </h3>
                  </Magnetic>
                  <p className="text-sm font-medium uppercase tracking-[0.2em] text-slate-400">{stat.label}</p>
               </div>
            ))}
         </div>
      </section>
      ...
      <section className="py-32 px-4 md:px-8">
         <div className="max-w-7xl mx-auto mb-20 fade-up">
            <h2 className="text-4xl md:text-6xl font-light tracking-tight text-black">Winning Masterpieces</h2>
            <p className="text-slate-500 mt-4 text-lg font-light">The pinnacle of artistic expression across three mediums.</p>
         </div>
         ...
         <div className="flex flex-col gap-32 max-w-[100rem] mx-auto">
            {winners.map((segment, idx) => (
               <div key={idx} className="fade-up flex flex-col gap-8">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-4 px-4">
                     <h3 className="text-2xl md:text-4xl font-light text-black">{segment.category}</h3>
                     <span className="text-sm uppercase tracking-widest text-slate-400">Top 3</span>
                  </div>
                  
                  <div className="flex flex-col md:flex-row h-[60vh] md:h-[70vh] gap-4 w-full">
                     {segment.images.map((img, i) => (
                        <div key={i} className="group relative flex-1 hover:flex-[3] transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] rounded-2xl md:rounded-3xl overflow-hidden cursor-pointer bg-slate-100">
                           <img 
                              src={img} 
                              alt={`${segment.category} Winner ${i + 1}`} 
                              className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 grayscale group-hover:grayscale-0 transition-all duration-700" 
                              referrerPolicy="no-referrer" 
                           />
                           ...
                           <div className="absolute top-6 right-6 md:top-8 md:right-8 w-12 h-12 rounded-full bg-white/50 backdrop-blur-md border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 -translate-y-4 group-hover:translate-y-0">
      ...
      <section className="py-32 px-6 md:px-24 max-w-7xl mx-auto border-t border-slate-200">
         <div className="fade-up mb-20 text-center">
            <h2 className="text-4xl md:text-6xl font-light tracking-tight text-black">The Curators</h2>
            <p className="text-slate-500 mt-4 text-lg font-light">The visionaries behind the exhibition.</p>
         </div>
            ...
            {ambassadors.map((ambassador, i) => (
               <Magnetic key={i}>
                  <div className="fade-up group relative flex flex-col items-center">
                     <div className="w-40 h-40 md:w-56 md:h-56 rounded-full overflow-hidden mb-6 border border-slate-200 transition-transform duration-500 group-hover:scale-105 group-hover:border-slate-300">
                        <img 
                           src={ambassador.img} 
                           alt={ambassador.name} 
                     ...
                     <h4 className="text-xl font-light text-black group-hover:text-[#00c8c8] transition-colors">{ambassador.name}</h4>
                     <p className="text-sm uppercase tracking-widest text-slate-400 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">{ambassador.role}</p>
                  </div>
               </Magnetic>
            ))}
         </div>
      ...
      {/* 5. PARTNERS */}
      <footer className="py-32 px-6 md:px-24 border-t border-slate-200">
         <div className="max-w-7xl mx-auto">
            <h3 className="text-center text-xs font-medium uppercase tracking-[0.3em] text-slate-400 mb-20">Institutional Partners</h3>
