         ...
         <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-24 items-start">
            ...
            <div className="lg:col-span-7">
               ...
               <div className="bg-white rounded-sm p-6 border border-slate-200 shadow-xl mb-10">
                 <div className="flex p-1 rounded-sm bg-slate-50 mb-8 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:none]">
                   {WINNERS_DATA.map((cat, idx) => (
                     <button
                       key={idx}
                       onClick={() => setActiveCategory(idx)}
                       className={`flex-1 py-3 px-4 rounded-sm text-xs font-mosaic font-bold uppercase tracking-wider transition-all whitespace-nowrap ${activeCategory === idx ? 'bg-[#ad944b] text-white shadow-md' : 'text-slate-500 hover:text-[#ad944b]'}`}
                     >
                       {cat.category}
                     </button>
                   ))}
