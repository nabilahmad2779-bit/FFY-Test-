  ...
  return (
    ...
    >
      ...
      >
         ...
         <div className={`absolute inset-[3px] rounded-[2.4rem] -z-10 ${isDark ? 'bg-[#050505]' : 'bg-white'}`} />
         ...
         <div className="relative z-10 p-7 md:p-10 flex flex-col h-full" style={{ transformStyle: 'preserve-3d' }}>
            ...
            <div className="transition-transform duration-300 group-hover:translate-z-12" style={{ transform: 'translateZ(40px)' }}>
               <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center mb-5 md:mb-8 border shadow-inner shrink-0 ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'}`} style={{ color: dept.accent }}>
                 {dept.icon}
               </div>
               <h4 className={`text-[8px] md:text-[10px] font-black uppercase tracking-widest mb-1.5 md:mb-3 leading-none ${isDark ? 'text-white opacity-60' : 'text-slate-500'}`}>{dept.tagline}</h4>
               <h3 className={`text-xl md:text-2xl font-heading font-black italic tracking-tight mb-3 md:mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>{dept.name}</h3>
            </div>

            {/* Layer 2: Description text - Moderate translation */}
            <p className={`text-xs md:text-sm font-light leading-relaxed mb-6 md:mb-8 transition-transform duration-300 ${isDark ? 'text-zinc-500' : 'text-slate-600'}`} style={{ transform: 'translateZ(25px)' }}>{dept.description}</p>
            ...
            <div className={`mt-auto pt-5 flex items-center justify-between transition-opacity ${isDark ? 'opacity-30 group-hover:opacity-100' : 'opacity-100'}`} style={{ transform: 'translateZ(15px)' }}>
               <span className={`text-[7px] md:text-[8px] font-black uppercase tracking-widest ${isDark ? 'text-white' : 'text-slate-400'}`}>Structural Nexus</span>
