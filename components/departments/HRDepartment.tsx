...
import { 
  ArrowLeft, Users, Heart, ArrowUpRight, 
        ...
        if (entries[0].isIntersecting && !hasAnimated) {
          ...
          const animate = (currentTime: number) => {
             ...
             if (progress < duration) {
                const ratio = easeOutQuart(progress / duration);
                setCount(Math.floor(end * ratio));
...
const HRDepartment: React.FC<DepartmentProps> = ({ navigate }) => {
  ...
  const theme = {
    ...
    navColor: isDark ? "text-white mix-blend-difference" : "text-[#2e1065]",
      ...
      >
        <div className={`fixed top-0 left-0 w-full z-[60] p-6 flex justify-between items-center transition-transform duration-500 ${showNav ? 'translate-y-0' : '-translate-y-full'} ${theme.navColor}`}>
          ...
          <div className="flex items-center gap-6">
             <button onClick={() => setIsDark(!isDark)} className={`${theme.navBtnHover} transition-colors p-2 rounded-full hover:bg-black/5`}>
        ...
        <section className={`relative h-screen w-full flex items-center justify-center overflow-hidden ${isDark ? 'bg-black' : 'bg-white'} transition-colors duration-700`}>
          <div className="absolute inset-0 z-0">
            ...
            <div 
              className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70vw] h-[70vw] bg-[#bf00ff] rounded-full blur-[160px] animate-pulse pointer-events-none transition-opacity duration-700 ${isDark ? 'opacity-20' : 'opacity-10'}`}
                   ...
                   <div className={`w-48 h-48 md:w-64 md:h-64 rounded-full p-2 bg-gradient-to-tr from-[#bf00ff] to-transparent shadow-[0_0_40px_rgba(191,0,255,0.1)] relative z-10 ${isDark ? 'shadow-black/50' : 'shadow-purple-200'}`}>
                      <div className={`w-full h-full rounded-full overflow-hidden ${isDark ? 'bg-black' : 'bg-white'} relative transition-colors duration-700`}>
                        <img 
                          ...
                          alt="Kashfia Anjum" 
                          className="w-full h-full object-cover object-top rounded-full grayscale hover:grayscale-0 transition-all duration-1000"
                        />
                      </div>
                   </div>
             ...
             <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 relative z-10">
                ...
                <div className="lg:col-span-8 flex flex-col gap-20 relative">
                     ...
                     {HR_STORY.map((item, index) => (
                         <div key={index} className="reveal-text group relative pl-4 md:pl-16">
                             <div className={`absolute left-[3px] md:left-[4px] top-2 w-2.5 h-2.5 rounded-full ${isDark ? 'bg-[#050505]' : 'bg-[#fdfaff]'} border-2 border-[#bf00ff] z-10 hidden md:block group-hover:bg-[#bf00ff] transition-all duration-300`} />
                             
                             <div className="relative z-10">
                                 <div className="flex items-center gap-4 mb-6">
                                     <div className={`flex items-center justify-center w-10 h-10 rounded-full border border-[#bf00ff]/20 ${isDark ? 'bg-white/5' : 'bg-purple-50'} text-[#bf00ff] group-hover:bg-[#bf00ff] group-hover:text-white transition-all duration-500`}>
                                        {item.icon}
                                     </div>
                                     <span className={`text-xs font-black uppercase tracking-[0.2em] ${theme.subText} group-hover:text-[#bf00ff] transition-colors`}>
                                         {item.year}
                                     </span>
                                 </div>
                                 
                                 <h3 className={`text-3xl md:text-5xl font-heading font-black uppercase italic mb-6 ${theme.text} group-hover:text-[#bf00ff] transition-colors leading-none`}>
                                     {item.title}
                                 </h3>
                                 
                                 <div className="relative">
                                     <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#bf00ff] scale-y-0 group-hover:scale-y-100 transition-transform duration-500 origin-top" />
                                     
                                     <p className={`text-lg md:text-xl font-light leading-relaxed text-justify ${theme.paragraphText} pl-6 border-l ${isDark ? 'border-white/10' : 'border-slate-200'} group-hover:border-transparent transition-colors`}>
          ...
          <section className="py-20 md:py-40 px-6 relative z-10 border-b border-[#bf00ff]/10 overflow-hidden">
             ...
             <div className="max-w-7xl mx-auto relative z-10">
                <div className="mb-20 text-center">
                   <h2 className={`reveal-text text-4xl md:text-7xl font-heading font-black uppercase mb-4 text-transparent bg-clip-text bg-gradient-to-r from-white via-purple-200 to-white animate-gradient-x`}>
                ...
                <div className="flex flex-wrap justify-center gap-x-12 gap-y-20 items-center">
                   {METRICS.map((item, i) => (
                     <div key={i} className="flex flex-col items-center justify-center text-center group relative p-10 transition-all duration-700 hover:scale-110">
                        <div className="absolute inset-0 bg-gradient-to-tr from-[#bf00ff]/20 to-cyan-500/20 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                        
                        <div className={`relative z-10 mb-6 transition-all duration-500 group-hover:-translate-y-4 group-hover:rotate-12 ${isDark ? 'text-zinc-600' : 'text-purple-200'} group-hover:text-white`}>
                            <item.icon size={64} strokeWidth={1.5} className="drop-shadow-[0_0_15px_rgba(191,0,255,0.5)]" />
                        </div>
                        
                        <h3 className="relative z-10 text-8xl md:text-[10rem] font-heading font-black tracking-tighter leading-none mb-4 text-transparent bg-clip-text bg-gradient-to-br from-white via-zinc-400 to-zinc-600 group-hover:from-[#bf00ff] group-hover:via-pink-500 group-hover:to-cyan-500 transition-all duration-700 select-none">
                            <CountUp end={item.value} suffix={item.suffix} />
                        </h3>
                        
                        <p className={`relative z-10 text-sm font-black uppercase tracking-[0.5em] ${theme.subText} group-hover:text-white transition-colors duration-500 bg-black/20 backdrop-blur-sm px-4 py-1 rounded-full border border-white/5 group-hover:border-white/20`}>
                           {item.label}
                        </p>
                     </div>
                   ))}
